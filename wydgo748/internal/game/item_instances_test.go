...
func TestInstanceMobCounterIsPresentInEncryptedOutboundPackets(t *testing.T) {
	w, leader, _, _, clock := instanceTestWorld()
	w.onUseItem(leader.Session, useItemPacket(0, 0))

	readCounter := func(session *wydnet.Session) (uint32, bool) {
		for {
			packet, ok := session.DequeuePacketForTest()
			if !ok {
				return 0, false
			}
			if !wire.Decrypt(packet) {
				t.Fatalf("outbound instance packet failed checksum")
			}
			header := wire.ParseHeader(packet)
			if header.Type != wire.OpInstanceMobs {
				continue
			}
			if header.Size != 16 {
				t.Fatalf("instance mob counter size=%d want=16", header.Size)
			}
			return binary.LittleEndian.Uint32(packet[12:16]), true
		}
	}

	if got, ok := readCounter(leader.Session); !ok || got != 1 {
		t.Fatalf("entry counter=%d present=%v want=1", got, ok)
	}
	var mob *Mob
	for id := range w.itemInstances["water-normal-1"].MobIDs {
		mob = w.mobsByID[id]
	}
	w.onItemInstanceMobKilled(mob, clock.Now())
	if got, ok := readCounter(leader.Session); !ok || got != 0 {
		t.Fatalf("kill counter=%d present=%v want=0", got, ok)
	}
}
