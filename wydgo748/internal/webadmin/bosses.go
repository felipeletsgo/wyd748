...
func (h *Handler) bosses(w http.ResponseWriter, r *http.Request) {
	...
	if staff.fingerprint() != s.StaffFingerprint || !staff.permits(control.BossesCapability) {
		h.audit(r, "bosses", s.Actor, "forbidden")
		writeError(w, 403, "capability_required")
		return
	}
	if h.cfg.Bosses == nil {
		writeError(w, 503, "bosses_unavailable")
		return
	}
	if !h.allow("bosses:"+s.Actor, 30) {
	...
	if r.Method == http.MethodGet {
		status, err := h.cfg.Bosses.BossesStatus(ctx)
		if err != nil {
	...
	authorize := func() bool {
		...
		if err != nil || !exists || entry.fingerprint() != s.StaffFingerprint || !entry.permits(control.BossesCapability) {
			return false
		}
		h.mu.Lock()
		defer h.mu.Unlock()
	...
	h.audit(r, "bosses."+cmd.Action, s.Actor, "requested", "operation_id", cmd.OperationID, "reason", cmd.Reason, "boss_id", cmd.BossID, "expected_revision", cmd.ExpectedRevision)
	result, err := h.cfg.Bosses.BossesCommand(ctx, s.Actor, cmd, authorize)
	if err != nil {
		h.audit(r, "bosses."+cmd.Action, s.Actor, "outcome_unknown", "operation_id", cmd.OperationID)
		globalDropTransportError(w, err)
		return
	}
	h.audit(r, "bosses."+cmd.Action, s.Actor, result.Code, "operation_id", cmd.OperationID, "replayed", result.Replayed)
