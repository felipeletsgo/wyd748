...
// ParseWYS decodifica o wrapper WS10 nativo 7.48 usado pelas texturas WYS.
// FUN_004b9d99 ignora o primeiro byte do arquivo, restaura "DDS" no buffer e
// decide o FourCC pelo byte 0x54: '2' significa DXT1 e qualquer outro valor
// significa DXT3. O decoder expande apenas o mip principal para RGBA8; os mips
// seguintes continuam no arquivo e nao participam do upload atual do OpenGL.
func ParseWYS(data []byte) (Texture, error) {
	...
	payloadSize := uint64(blocksWide) * uint64(blocksHigh) * uint64(blockSize)
	...
	for blockY := uint32(0); blockY < blocksHigh; blockY++ {
		for blockX := uint32(0); blockX < blocksWide; blockX++ {
			off := (blockY*blocksWide + blockX) * blockSize
			if isDXT1 {
				decodeDXT1Block(pixels, width32, height32, blockX, blockY, payload[off:off+8])
			} else {
				decodeDXT3Block(pixels, width32, height32, blockX, blockY, payload[off:off+16])
			}
		}
	}
...
func decodeDXT1Block(dst []byte, width, height, blockX, blockY uint32, block []byte) {
	colors := dxtColorPalette(block, true)
...
// decodeDXT3Block aplica o alpha explicito de 4 bits do DXT3 e usa a paleta
// de quatro cores obrigatoria desse formato, inclusive quando c0 <= c1.
func decodeDXT3Block(dst []byte, width, height, blockX, blockY uint32, block []byte) {
	alpha := binary.LittleEndian.Uint64(block[0:8])
	colors := dxtColorPalette(block[8:16], false)
	...
	for py := uint32(0); py < 4; py++ {
		...
		for px := uint32(0); px < 4; px++ {
			...
			pixel := py*4 + px
			selector := (indices >> (2 * pixel)) & 0x3
			out := (y*width + x) * 4
			copy(dst[out:out+4], colors[selector][:])
			dst[out+3] = uint8((alpha>>(4*pixel))&0x0f) * 17
		}
	}
}

func dxtColorPalette(block []byte, allowDXT1Transparency bool) [4][4]uint8 {
	...
	if !allowDXT1Transparency || c0 > c1 {
