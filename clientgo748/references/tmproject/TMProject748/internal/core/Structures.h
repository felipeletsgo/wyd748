...
struct TMVector3
{
	...
	{
		// Empty-brace assignments (for example, `vector = {}`) bind to this
		// legacy pointer overload. Treat a null source as a zero vector instead
		// of dereferencing it; this is required by effect constructors such as
		// TMEffectFireWork and preserves the existing pointer-copy behavior.
		if (!other)
		{
			x = 0.0f;
			y = 0.0f;
