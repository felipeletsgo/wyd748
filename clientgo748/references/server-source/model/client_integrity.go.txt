...
// isSafeClientAssetPath aplica a mesma regra nos runners Windows e Unix. O
// manifesto acompanha assets do client 7.48, portanto separadores invertidos,
// letras de unidade e UNC precisam ser tratados explicitamente em qualquer
// sistema onde o servidor seja validado.
func isSafeClientAssetPath(asset string) bool {
	asset = strings.TrimSpace(asset)
	if asset == "" || strings.IndexByte(asset, 0) >= 0 {
		return false
	}
	normalized := strings.ReplaceAll(asset, `\`, "/")
	if strings.HasPrefix(normalized, "/") ||
		(len(normalized) >= 2 && ((normalized[0] >= 'A' && normalized[0] <= 'Z') ||
			(normalized[0] >= 'a' && normalized[0] <= 'z')) && normalized[1] == ':') {
		return false
	}
...
func (f ClientIntegrityFile) Validate() error {
	...
	for i, probe := range f.Probes {
		...
		if !isSafeClientAssetPath(asset) {
