...
def main() -> int:
    ...
    parser.add_argument(
        "--flows",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "references" / "research" / "flows",
        help="diretório autocontido das fichas; por padrão usa clientgo748/references/research/flows",
    )
    args = parser.parse_args()
    root = args.flows.resolve()
    ...
    for path in files:
        ...
        if errors:
            failures += 1
            for error in errors:
                print(f"ERROR {path.relative_to(root.parent.parent.parent)}: {error}")
        else:
            print(f"OK {path.relative_to(root.parent.parent.parent)}: {status}")
