...
void MobKilled(int target, int conn, int PosX, int PosY)
{
	...
	if ((conn > 0 && conn < MAX_USER) && (target > 0 && target < MAX_USER)) {
		if ((conn == Desafiantes[0] || conn == Desafiantes[1]) && (target == Desafiantes[0] || target == Desafiantes[1])) {
			...
			SendClientMessage(target, "Infelizmente voc? perdeu...");
	...
	#pragma region >> Func para quem morreu
			...
			//			//pUser[conn].Honra += 5;
			//			SendClientMessage(target, "Recebeu um pr?mio de consolo da Arena Real.");						
			//			DoTeleport(target, 2132 + rand() % 5, 2147 + rand() % 5);
			//			return;
			//		}
			//	}
...
#pragma region Sombra Negra
						if (pMob[target].GenerateIndex == SOMBRANEGRA)
						{
							...
							if (PutItem(conn, &item)){
								SendClientMessage(conn, "Voc? recebeu [01][Barra_de_Prata(50Mi)]");
...
#pragma region Verid Negro
						if (pMob[target].GenerateIndex == VERIDNEGRO)
						{
							...
							if (PutItem(conn, &item))
								SendClientMessage(conn, "Voc? recebeu [02][Barra_de_Prata(50Mi)]");
							g_BVerid = FALSE;
...
#pragma endregion
						...
						//							DoRecall(conn);
						//
						//							SendClientMessage(conn, "Parab?ns voc? concluiu a Quest.");
						//							for (int i = 0; i < MAX_PARTY; i++)
						//							{
						...
						//									DoRecall(partyconn);
						//									EspelhoLive = 0;
						//									SendMsgExp(partyconn, "Parab?ns voc?s concluiram a Quest.", TNColor::Default, false);
						//								}
						//							}
						//						}
						//#pragma endregion
...
#pragma region Port?es de Noatum
...
#pragma region Agua M
								...
								if (GenerateID >= WATER_M_INITIAL && GenerateID <= WATER_M_INITIAL + 7)
								{
									...
									if (CurrentNumMob == 1)
									{
										...
										for (int i = 0; i < MAX_PARTY; i++)
										{
											int partymember = pMob[partyleader].PartyList[i]; // tu n?o pagou mas vou corrigir, isso acontece na agua tbm
...
#pragma endregion
#pragma region Port?o Infernal Parte 1
								else if (GenerateIndex >= PI_INITIAL1 && GenerateIndex <= PI_END1)
								{
									// Port?o Infernal Parte 1
									GenerateMob(GenerateIndex, 0, 0);
								}
#pragma endregion
#pragma region Carta de duelo N - WAVE SYSTEM
								//Carta N - Sistema de Ondas com Instâncias
								// PRIMEIRO verificar se está em área de instância ativa
								int mobX = pMob[target].TargetX;
								int mobY = pMob[target].TargetY;
								int instanceID = GetCartaInstanceByPosition(mobX, mobY);

								if (instanceID >= 0 && CartaNInstances[instanceID].Active)
								{
									// DEPOIS verificar se é um mob do Wave System (range 5051-5062)
									if (GenerateID >= CARTA_N_WAVE_MOB_START && GenerateID <= CARTA_N_WAVE_MOB_END)
									{
										CartaNInstances[instanceID].MobsKilled++;

										int remaining = CartaNInstances[instanceID].MobsTotal - CartaNInstances[instanceID].MobsKilled;

										// Enviar contador para o grupo
										SendMobCounter(instanceID, remaining);

										// Verificar se onda terminou
										if (CartaNInstances[instanceID].MobsKilled >= CartaNInstances[instanceID].MobsTotal)
										{
											if (CartaNInstances[instanceID].CurrentWave >= 4)
											{
												// Boss morto - finalizar instância
												CompleteCartaNInstance(instanceID);
											}
											else
											{
												// Todos os mobs mortos - próxima onda imediatamente
												CartaNInstances[instanceID].CurrentWave++;
												SpawnCartaNWave(instanceID);

												// Enviar mensagem ao grupo
												for (int i = 0; i < MAX_PARTY; i++)
												{
													int partyconn = pMob[CartaNInstances[instanceID].LeaderConn].PartyList[i];
													if (partyconn > 0 && partyconn < MAX_USER && pUser[partyconn].Mode == USER_PLAY)
													{
														char msg[128];
														sprintf(msg, "Onda %d iniciada!", CartaNInstances[instanceID].CurrentWave);
...
#pragma region Drop Gold
							...
							if (pMob[target].MOB.BaseScore.Level < 10)
								...
								UNKGOLD = 9; // aparentementer corrigido man, ta igual ao meu e eu n?o tenho mais esse b.o se ainda causar, me avisa que a gente v? :) ok
