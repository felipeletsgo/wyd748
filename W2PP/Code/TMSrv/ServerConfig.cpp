...
void ReadServerConfigs()
{
	...
	SNPRINTF_SAFE(hQuery, sizeof(hQuery), "SELECT * FROM `serverconfig` WHERE `id` = '%d'", 1);
	MYSQL_ROW row;
...
void ReadExpTableN() {
	...
	SNPRINTF_SAFE(hQuery, sizeof(hQuery), "SELECT * FROM `expbase` WHERE `id` = '%d'", 1);
	MYSQL_ROW row;
	MYSQL* wSQL = pc.wStart();
	MYSQL_RES* result = pc.wRes(wSQL, hQuery);
...
void ReadExpTableM() {
	...
	SNPRINTF_SAFE(hQuery, sizeof(hQuery), "SELECT * FROM `expbase` WHERE `id` = '%d'", 2);
	MYSQL_ROW row;
	MYSQL* wSQL = pc.wStart();
	MYSQL_RES* result = pc.wRes(wSQL, hQuery);
