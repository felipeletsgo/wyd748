...
void MailItem(int conn)
{
	...
	SNPRINTF_SAFE(hQuery, sizeof(hQuery), "SELECT * FROM `mailitem` WHERE `account` = '%s' AND `delivered` = '%d' LIMIT 1", escapedAccount, 0);
	MYSQL_ROW row;
...
void SendMailItem(int conn)
{
	...
	mysql_free_result(result);
	if (Index == 0) {
		SendClientMessage(conn, "N�o h� itens a serem recebidos");
		return;
	}
	...
	SendClientMessage(conn, strFmt("O item [%s] foi entregue no seu invent�rio!", g_pItemList[Index].Name));
