...
void Exec_MSG_Buy(int conn, char* pMsg)
{
	...
	{
		SendClientMessage(conn, "Imposs�vel executar a��o7, tente mais tarde. ");
		return;
	}
	...
	if (strcmp(pMob[TargetID].MOB.MobName, "Thorin") == 0) //npc pvp armia batalha real
	{
		...
		//		{
		//			SendSay(itemIndex, strFmt("Aguarde [%d] minutos para o in�cio da batalha.", 5 - now.tm_min));
		//			return;
		//		}
		...
		//	{
		//		SendClientMessage(conn, "N�o � permitido registro em grupo.");
		//		return;
		//	}
		//	int i = 0;
		...
		//		else
		//		{
		//			SendClientMessage(conn, "O registro � permitido de segunda a s�bado do minuto 0 ao 5");
		//			return;
		//		}
		...
		//	else
		//	{
		//		SendClientMessage(conn, "N�o � poss�vel batalhar nos fins de semana.");
		//		return;
		//	}
		//	return;
		//}
		...
		for (int x = 0; x < Qnt; x++) {
			if (itemIndex == ItemEntregue[x][0]) {
				...
				if (pUser[conn].Lasclick != itemIndex) {
					SendClientMessage(conn, strFmt("Voc� precisa de %d %s. Deseja prosseguir?", ItemSolicitado[x][1], g_pItemList[ItemSolicitado[x][0]].Name));
				...
				if (RemoveItem(conn, ItemSolicitado[x][0], ItemSolicitado[x][1]) == true) {
					...
					if (PutItem(conn, &Item) == false) {
						ItemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, 
							strFmt("Exchange - N�o houve espa�o para entregar [%d]x[%d]", ItemEntregue[x][1], ItemEntregue[x][0]));
					}
				}
				else {
					SendClientMessage(conn, strFmt("Voc� precisa de %d %s, para efetuar essa troca",
	...
	if (Donate)
	{
		/*SendClientMessage(conn, "A compra de itens donate ser� liberado no s�bado 12h");
		return;*/
	...
	if (Honra)
	{
		...
		if (pos >= 0 && pos < pMob[conn].MaxCarry && pos < MAX_CARRY)
		{
			...
			{
				SendItemagruparEx(conn, ItemMob->sIndex, Qnt);
			}
			else
				PutItem(conn, &Item);
