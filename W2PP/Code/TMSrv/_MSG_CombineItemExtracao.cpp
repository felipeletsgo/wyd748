...
void Exec_MSG_CombineItemExtracao(int conn, char *pMsg)
{
	...
	{
		SendClientMessage(conn, "Imposs�vel executar a��o17, tente mais tarde. ");
		return;
	}
	...
	if(random < rate)
	{
		...
		STRNCAT_SAFE(temp, tt, sizeof(temp));

		//ItemLog(temp, "*** Combine ht_skill_extra��o sucess ***", pUser[conn].IP);
	}
	else
	{
		...
		//ItemLog("*** Combine ht_skill_extra��o fail ***", pMob[conn].MOB.MobName, pUser[conn].IP);
	}
}