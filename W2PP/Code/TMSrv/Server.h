...
// Estrutura para rastrear cada instância de Carta N
struct STRUCT_CARTA_N_INSTANCE
{
	int Active;           // Se instância está ativa (0/1)
	int LeaderConn;       // Conexão do líder do grupo
	int PartyID;          // ID do party (para rastrear membros)
	int CurrentWave;      // Onda atual (1-4)
	int WaveTime;         // Tempo restante para próxima onda
	int MobsKilled;       // Mobs mortos na onda atual
	int MobsTotal;        // Total de mobs na onda atual
	int SpawnX, SpawnY;   // Coordenada de spawn da instância
};

extern STRUCT_CARTA_N_INSTANCE CartaNInstances[MAX_CARTA_N_INSTANCES];
extern int CartaNInstancePos[MAX_CARTA_N_INSTANCES][2];
