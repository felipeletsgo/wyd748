	...
	if (level != 2000)
	{
		...
		if (!strcmp(cmd, "set"))
		{
			...
			else if (!strcmp(sval1, "donate"))
			{
				...
				SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE accounts SET donate = '%d' WHERE username = '%s' ", ival2, escapedAccount);
				pc.wQuery(xQuery);
	...
	if (level >= 1)
	{
		...
		else if (!strcmp(cmd, "gfame"))
		{
			...
			if (ival1 > 0 && ival1 < 65536)
			{
				...
				auto& pc = cSQL::instance();
				SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE `guilds` SET fame = '%d' WHERE `guild` = '%d'", ival2, ival1);
				pc.wQuery(xQuery);
