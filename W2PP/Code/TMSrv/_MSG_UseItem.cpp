...
void Exec_MSG_UseItem(int conn, char* pMsg)
{
	...
	{
		SendClientMessage(conn, "Impossvel executar ao54, tente mais tarde. ");
		return;
	}
...
#pragma region >> Replation Normal
	if (item->sIndex >= 4016 && item->sIndex <= 4020)
	{
		...
		{
			SendClientMessage(conn, "No  possvel utilizar em cytheras.");
		...
		{
			SendClientMessage(conn, "Possvel somente para equipamentos de defesa.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region PO / PL
	if (Vol == 4 || Vol == 5) // PO - PL
	{
		...
		{
			SendClientMessage(conn, "Esse item no pode ser refinado");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Pedras Arch
		if (dest->sIndex >= 1752 && dest->sIndex <= 1759)
		{
			...
			else if (dest->sIndex == 1754)//Drago Lich
			{
				if (_rd < 3)
					NextPedra = 1744;//Inteligencia
...
#pragma region AMULETOS PREMIUM
		...
		if (item->sIndex == 412) {
			if (dest->sIndex >= 762 && dest->sIndex <= 767) {
				SendClientMessage(conn, "Este Item no pode ser refinado com Poeira de Ori");
				return;
			}
		}
...
#pragma region Poeira de Fada
	if (Vol == 7)//Poeira de Fada
	{
		...
		{
			SendClientMessage(conn, "Desculpe. Item disponvel apenas para Mortais .");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Poo Kappa
	if (Vol == 10 || Vol == 55 || Vol == 200 || Vol == 201 || Vol == 56 || Vol == 52 || Vol == 53 || Vol == 57 || Vol == 202)
	{
		...
		/*
		snprintf(temp, sizeof(temp), "useitem,poo_kappa affectslot:%d value:%d time:%d", EmptyAffect, value, tempo);
		ItemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, temp);*/
		return;
	}
#pragma endregion
...
#pragma region Portal
	if (Vol == 13)
	{
		...
		{
			SendClientMessage(conn, "rea de Destino Invlida");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Raes
	if (Vol == 15)
	{
		...
		/*
		snprintf(temp, sizeof(temp), "useitem,rao mount:%d", mount);
		ItemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, temp);*/
		return;
	}
#pragma endregion
	...
	if (Vol == 16)
	{
		...
		if (dest->sIndex >= 2360 && dest->sIndex < 2390 || dest->sIndex < 3091 && dest->sIndex >= 3139)
		{
			...
			{   //bama adicionou
				...
				SendClientMessage(conn, "Obteve sucesso na refinao.");
			}
			...
			|| level >= 100 && dest->sIndex >= 2336 && dest->sIndex < 2360)
		{
			...
			if (dest->stEffect[1].cValue >= 60) // Verificao para saber se a vitalidade  maior que 60
				dest->stEffect[1].cValue = 60; // iguala a vitalidade a 60 para no bugar
...
#pragma region Livros TRANSKNIGHT
	if (item->sIndex == 5500) // Proteo Divina
	{
		...
		{
			SendMsgExp(conn, " necessrio ser Celestial/SubCelestial.", TNColor::Default, false);
		...
		if (pMob[conn].MOB.Class == 0)
		{
			...
			{
				SendMsgExp(conn, "Apenas uma 9 Skill por personagem.", TNColor::Default, false);
			...
			{
				SendMsgExp(conn, "Necessrio ter aprendido a 8 Skill.", TNColor::Default, false);
		...
		{
			SendMsgExp(conn, "Esse livro no pertence a sua Classe.", TNColor::Default, false);

			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
	}
	if (item->sIndex == 5501) // Bno Divina
	{
		int ItemID = item->sIndex;
		...
		if (pMob[conn].MOB.Class == 0)
		{
			...
			{
				SendMsgExp(conn, "Apenas uma 10 Skill por personagem.", TNColor::Default, false);
			...
			{
				SendMsgExp(conn, "Necessrio ter aprendido a 9 Skill.", TNColor::Default, false);
	...
	if (item->sIndex == 5504) // Mos Sangrentas
	{
		if (pMob[conn].MOB.BaseScore.Level <= 199)
		{
			SendMsgExp(conn, "Level insuficiente.", TNColor::Default, false);
	...
	if (item->sIndex == 5508) // Espelho Mgico
	{
	...
	if (item->sIndex == 5509) // Conexo de Gelo
	{
		int ItemID = item->sIndex;
...
#pragma region Livros FOEMA
	...
	if (item->sIndex == 5513) // Proteo Absoluta
	{
		int ItemID = item->sIndex;
...
#pragma region Livros BEASTMASTER
	...
	if (item->sIndex == 5529) // Invocao Final
	{
		int ItemID = item->sIndex;
...
#pragma region Livros HUNTRESS
	...
	if (item->sIndex == 5537) // Ataque Rpido Proficiente
	{
		int ItemID = item->sIndex;
	...
	if (item->sIndex == 5541) // Absoro de Alma
	{
		int ItemID = item->sIndex;
...
#pragma region Pergaminho da agua M
	if (Vol >= 21 && Vol <= 30)
	{
		...
		if (UserArea >= 1)
		{
			SendMsgExp(conn, strFmt("O jogador [%s] J se encontra na sala", UserName), TNColor::Default, false);
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region >> Poo Divina 7 15 30 Dias 
...
#pragma endregion
#pragma region Removedor de Pergaminho da Transformao
	if (item->sIndex == 3335)
	{
		...
		{
			SendMsgExp(conn, "Voce no est transformado!", TNColor::Default, false);
...
#pragma region Entrada Cassino // Passe Evento
	if (Vol == 178)
	{
		if (pUser[conn].Keys[49] == 1) {
			SendClientMessage(conn, "Passe do Evento j est ativo");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma endregion
#pragma region Porto Infernal N/M (Ind/Grupo)
	if (Vol == 176)
	{
		int TargetX = pMob[conn].TargetX;
		int TargetY = pMob[conn].TargetY;
...
#pragma region Carta de Duelo N.M.A
	if (Vol == 20)
	{
		...
		// Custo removido - Carta N é grátis
		if (pMob[conn].MOB.Coin < 0) {
			SendClientMessage(conn, "Erro ao verificar gold.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
		...
		// ========================================================================
		// CARTA N - SISTEMA DE ONDAS COM 3 INSTÂNCIAS
		// ========================================================================
		if (cartaindex == 1)
		{//Carta N - Wave System
			// Encontrar instância disponível (prioridade: 1 → 2 → 3)
			int allocatedInstance = -1;
			for (int i = 0; i < MAX_CARTA_N_INSTANCES; i++)
			{
				// Verificar se instância está inativa
				if (CartaNInstances[i].Active == 0)
				{
					allocatedInstance = i;
					break;
				}
			...
			if (allocatedInstance == -1)
			{
				SendClientMessage(conn, "Todas as instancias da Carta N estao ocupadas. Tente novamente mais tarde.");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}

			// Inicializar instância
			CartaNInstances[allocatedInstance].Active = 1;
			CartaNInstances[allocatedInstance].LeaderConn = conn;
			CartaNInstances[allocatedInstance].PartyID = 0; // Não usado no momento
			CartaNInstances[allocatedInstance].CurrentWave = 1;
			CartaNInstances[allocatedInstance].WaveTime = 60;  // 1 minuto por onda (fallback se nao matar todos)
			CartaNInstances[allocatedInstance].MobsKilled = 0;
			CartaNInstances[allocatedInstance].MobsTotal = 50;   // 50 mobs na onda 1
			CartaNInstances[allocatedInstance].SpawnX = CartaNInstancePos[allocatedInstance][0];
			CartaNInstances[allocatedInstance].SpawnY = CartaNInstancePos[allocatedInstance][1];

			// Custo removido - Carta N é grátis
			// pMob[conn].MOB.Coin -= 0;

			// Teleportar líder para a instância
			DoTeleport(conn, CartaNInstances[allocatedInstance].SpawnX, CartaNInstances[allocatedInstance].SpawnY);
			char msg[128];
			sprintf(msg, "Carta N - Instancia %d - Onda 1 iniciada! Monstros: 50", allocatedInstance + 1);
			SendClientMessage(conn, msg);

			// Teleportar membros do party
			for (int i = 0; i < MAX_PARTY; i++)
			{
				int partyconn = pMob[conn].PartyList[i];
				...
				if (partyconn > 0 && partyconn < MAX_USER && partyconn != conn && pUser[partyconn].Mode == USER_PLAY)
				{
					DoTeleport(partyconn, CartaNInstances[allocatedInstance].SpawnX, CartaNInstances[allocatedInstance].SpawnY);
					sprintf(msg, "Carta N - Instancia %d - Onda 1 iniciada! Monstros: 50", allocatedInstance + 1);
					SendClientMessage(partyconn, msg);
				}
			}

			// Limpar mobs da área da instância
			ClearCartaInstanceMobs(allocatedInstance);

			// Spawnar onda 1
			SpawnCartaNWave(allocatedInstance);
		}
		// ========================================================================
		// FIM DO SISTEMA DE ONDAS DA CARTA N
		// ========================================================================

		else if (cartaindex == 0)
		{//Carta M (sistema antigo)
			char UserName[128];
			...
			// Custo removido - Cartas são grátis
...
#pragma region >> Retornar Cavalo
	if (item->sIndex == 3453)
	{
		...
		{
			SendClientMessage(conn, "?Sua montaria no est ferida.?");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Pedra Ideal
	if (Vol == 211)
	{
		...
		{
			SendClientMessage(conn, "Criao de Celestial Bloqueada");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region HardCore Desligado
		if (isHardCore == 0)
		{
			...
			{
				SendClientMessage(conn, "No h quest's disponveis para sua classe.");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
			...
			{
				SendClientMessage(conn, "Somente personagem nvel 400.");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
			...
			{
				SendClientMessage(conn, "Equipe a o item (Sephirot[Classe]) para conclur esta Quest.");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
			...
			{
				SendClientMessage(conn, "Seu Persogaem deve possuir nivel 121 + !"); //"Somente personagem nvel 120.");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
			...
			{
				SendClientMessage(conn, "Seu personagem no possui [100] de Fame.");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
		...
		else if (isHardCore == 1)
		{
			...
			if (pMob[conn].extra.ClassMaster == CELESTIAL)
			{
				...
				else if (pMob[conn].MOB.Equip[10].sIndex == 1742 && (pMob[conn].MOB.Equip[11].sIndex != 1760 && pMob[conn].MOB.Equip[11].sIndex != 1761 && pMob[conn].MOB.Equip[11].sIndex != 1762 && pMob[conn].MOB.Equip[11].sIndex != 1763))
				{
					...
					SendClientMessage(conn, "Divino no pode ser criado por Celestial a partir da verso 7.55");
					return;
				}
			}
	...
	if (Vol == 187)
	{
		...
		if (pMob[conn].extra.ClassMaster == ARCH || pMob[conn].extra.ClassMaster == HARDCOREA)
		{
			if (pMob[conn].extra.ClassMaster == ARCH)
			{
				...
				{
					SendClientMessage(conn, "Quest no pode ser concluda no nvel 370.");
					SendItem(conn, m->SourType, m->SourPos, item);
					return;
				}
...
#pragma region Pedra da Fria
	if (item->sIndex == 3020)
	{
		...
		{
			if (!pMob[conn].MOB.Equip[1].sIndex) {
				SendClientMessage(conn, " Preciso estar com a Cythera equipada");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
		...
		else
		{
				...
				pMob[conn].MOB.CurrentScore.Level == 319 || pMob[conn].MOB.CurrentScore.Level == 359)
			{
				if (pMob[conn].MOB.CurrentScore.Level == 239)
				{
					...
					{
						SendClientMessage(conn, "Falha no destrave de nvel.");
						if (amount > 1)
							BASE_SetItemAmount(item, amount - 1);
...
#pragma endregion
#pragma region Sussurro Demonaco
	if (item->sIndex == 3478)
	{
		if (pMob[conn].extra.ClassMaster < CELESTIAL || pMob[conn].extra.ClassMaster > CELESTIALCS)
		{
...
#pragma endregion
#pragma region Remdio da Coragem
...
#pragma region Feijo magico
	if (Vol == 186)
	{
		...
		if (sanc < 7)
		{
			SendClientMessage(conn, "Possvel somente para equipamentos acima de +6.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Pergaminho do Perdo
	if (Vol == 203)
	{
		SetPKPoint(conn, 150);
		...
		SendEmotion(conn, 14, 3);
		pMob[conn].extra.EMPTY[0] = 0; //remove puniao cp
		pMob[conn].extra.EMPTY[1] = 0; //remove puniao cp
		pMob[conn].GetCurrentScore(conn);
		SendScore(conn);
		SendEtc(conn);
		SendClientMessage(conn, "Punies CP foram removidas");
		SaveUser(conn, 0);
...
#pragma region Gema de diamante
	if (Vol == 180)
	{
		...
		// Itens que NÃO podem receber gemas
		...
		// Cintos (4478-4481) e Colares (4493-4494) podem receber gemas mesmo com Grade < 5
		bool isCintoOrColar = (ItemId >= 4478 && ItemId <= 4481) || (ItemId >= 4493 && ItemId <= 4494);

		if (!isCintoOrColar && sanc < REF_10 && g_pItemList[ItemId].Grade < 5)
		{
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Gema de Coral
	if (Vol == 182)
	{
		...
		// Cintos (4478-4481) e Colares (4493-4494) podem receber gemas mesmo com Grade < 5
		bool isCintoOrColar_Coral = (ItemId >= 4478 && ItemId <= 4481) || (ItemId >= 4493 && ItemId <= 4494);

		if (!isCintoOrColar_Coral && sanc < REF_10 && g_pItemList[ItemId].Grade < 5)
		{
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Gema de Garnet
	if (Vol == 183)
	{
		...
		// Cintos (4478-4481) e Colares (4493-4494) podem receber gemas mesmo com Grade < 5
		bool isCintoOrColar_Garnet = (ItemId >= 4478 && ItemId <= 4481) || (ItemId >= 4493 && ItemId <= 4494);

		if (!isCintoOrColar_Garnet && sanc < REF_10 && g_pItemList[ItemId].Grade < 5)
		{
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma endregion
#pragma region Entrada do Territrio
	if (Vol == 188)
	{
...
#pragma endregion
#pragma region Armazenagem - Recuperao
	if (Vol == 243)
	{
		if (item->sIndex == 3203)
		{
...
#pragma region >> Ba_de_Secretas
	if (item->sIndex == 5850)
	{
		// Nova lista com 4 opes de itens
		struct ItemOption {
			int sIndex;
			int effect1;
			int value1;
...
#pragma region Caixa do iniciante
	if (item->sIndex == 5971)
	{
		...
		SendCarry(conn);
		SendClientMessage(conn, "Voce recebeu o kit migrao!");
...
#pragma region Bolao da Guild
	if (item->sIndex == 5989)
	{
		...
		SendCarry(conn);
		SendClientMessage(conn, "Voce recebeu o kit Bolao GUild, Obrigado pelo apoio!");

		return;
	}
#pragma endregion
#pragma region Caixa de Planeta Aleatrio
	if (item->sIndex == 5773)
	{
		STRUCT_ITEM Item;
		memset(&Item, 0, sizeof(STRUCT_ITEM));
...
#pragma region >> Catalizadores
	if (Vol == 94)
	{
		...
		// Kapel
		if (mount >= 2333 && mount <= 2335) //Dente de Sabre, Drago Menor, Urso
			mount = 0;
		...
		// Birago
		if (mount >= 2346 && mount <= 2348) //Drago, Fenri das Sombras, Fenri
			mount = 3;
		...
		// Alperath
		if (mount >= 2349 && mount <= 2350) //Drago Vermelho, Tigre de Fogo
			mount = 6;
...
#pragma region Restauradores
	if (Vol == 93)
	{
		...
		{
			SendItem(conn, m->SourType, m->SourPos, item);
			SendClientMessage(conn, "Vitalidade da montaria est no mximo.");
			return;
		}
		...
		{
			SendItem(conn, m->SourType, m->SourPos, item);
			SendClientMessage(conn, "Usado apenas nas montarias citadas na descrio.");
			return;
		}
...
#pragma endregion
#pragma region Mandado de Exlio
	if (item->sIndex == 5602)
	{
		...
		for (int a = 0; a < 3; a++) {
			if (dest->stEffect[a].cEffect == EF_DAMAGE || dest->stEffect[a].cEffect == EF_DAMAGE2) {
				...
				if (dest->stEffect[a].cValue < (m->DestPos != 6 && m->DestPos != 7 ? 12 : 24)) {
					SendClientMessage(conn, "No  possvel aprimorar este item.");
					SendItem(conn, m->SourType, m->SourPos, item);
					return;
				}
			}
...
#pragma region Barra Mytril (Defesa)
	if (Vol == 237)
	{
		...
		if (dest->sIndex >= 3500 && dest->sIndex <= 3507)
		{
			SendClientMessage(conn, "Impossvel Utilizar com Cytheras");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma endregion
#pragma region Barra Mytril (Crtico)
	if (Vol == 238)
	{
		STRUCT_ITEM* dest = GetItemPointer(&pMob[conn].MOB, pUser[conn].Cargo, m->DestType, m->DestPos);
...
#pragma region Donate
	if (Vol == 189)
	{
		...
		if (item->sIndex == 3394)
		{
			SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE accounts SET donate = '%d' WHERE username = '%s' ", (Donate + 100), escapedAccount);
			pc.wQuery(xQuery);

			SendMsgExp(conn, "Voce ativou 100 de Donate.", TNColor::GoldenClaro, false);
		...
		{
			SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE accounts SET donate = '%d' WHERE username = '%s' ", (Donate + 1000), escapedAccount);
			pc.wQuery(xQuery);

			SendMsgExp(conn, "Voce ativou 1.000 de Donate.", TNColor::GoldenClaro, false);
		...
		{
			SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE accounts SET donate = '%d' WHERE username = '%s' ", (Donate + 5000), escapedAccount);
			pc.wQuery(xQuery);

			SendMsgExp(conn, "Voce ativou 5.000 de Donate.", TNColor::GoldenClaro, false);
...
#pragma endregion 
#pragma region extraes(Item)
#pragma region extrao Elmo
	if (item->sIndex == 3021)
	{
		if (pMob[conn].MOB.Equip[1].sIndex == 0)
		{
...
#pragma region >> extrao Elmo
	if (item->sIndex == 3021)
	{
		...
		if (pMob[conn].MOB.Equip[1].sIndex >= 3500 && pMob[conn].MOB.Equip[1].sIndex <= 3507)
		{
			SendClientMessage(conn, "No  possivel utilizar em Cytheras.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
	...
	if (item->sIndex == 3026)
	{
		if (item->stEffect[0].cEffect == 0 && item->stEffect[0].cValue == 0 && item->stEffect[1].cEffect == 0 && item->stEffect[1].cValue == 0 && item->stEffect[2].cEffect == 0 && item->stEffect[2].cValue == 0) {
			...
			if (_rd < 25) {
				...
				SendEmotion(conn, 14, 3);
				SendClientMessage(conn, "Extrado com sucesso.");
				SendCarry(conn);
				return;
			}
			if (_rd > 25) {
...
#pragma region Transmutaes 
	if (item->sIndex == 5614)
	{
		if (pMob[conn].extra.ClassMaster == MORTAL)
		{
			SendClientMessage(conn, "Sua classe atual no  permitida.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Composto Equilibrio
	if (Vol == 193)
	{
		...
		{
			SendClientMessage(conn, "Voce j completou essa quest!");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
		if (pMob[conn].extra.ClassMaster != MORTAL)
		...
		if (pMob[conn].MOB.CurrentScore.Level < minlevel || pMob[conn].MOB.CurrentScore.Level >= maxlevel)
		{
			SendClientMessage(conn, "Seu nivel  insuficiente.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma endregion
#pragma region Ba Novato
	if (item->sIndex == 5644) // Ba Novato
	{
		int x = 0;
		int invfree = 0;
		// Random para buscar e verificar os slots do inventrio. 
		...
		if (invfree < 5)
		{
			SendClientMessage(conn, "Seu inventrio est cheio.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma endregion
#pragma region Forsa PT 300 // Ba Novato
	if (item->sIndex == 5611)
	{
		pUser[conn].Donate += 300;

		SendMsgExp(conn, "Voce ativou 300 de Donate.", TNColor::GoldenClaro, false);
...
#pragma region Cubo da Maldade
	if (item->sIndex == 1772)
	{
		...
		if (pMob[conn].TargetX >= 1810 && pMob[conn].TargetY >= 3859 && pMob[conn].TargetX <= 1811 && pMob[conn].TargetY <= 3860)
		{
			if (CuboN.pRoom >= 6)
			{
				SendMsgExp(conn, "Esta quest atingiu o limite mximo de jogadores!", TNColor::Default, false);
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
...
#pragma endregion
	if (item->sIndex == 5571)
	{
		...
		{
			SendClientMessage(conn, "No  possivel utilizar nesses itens.");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma endregion
#pragma region >> Ba de Amuleto Arcano
	if (item->sIndex == 5633)
	{
		int rand_ = rand() % 100;
		STRUCT_ITEM Item;
...
#pragma region
	if (item->sIndex == 3392)
	{
		SendClientMessage(conn, "No permitido o uso do pesa individual");
		SendItem(conn, m->SourType, m->SourPos, &pMob[conn].MOB.Carry[m->SourPos]);
...
#pragma region
	if (item->sIndex == 3326)
	{
		SendClientMessage(conn, "No permitido o uso do pesa Grupo");
		SendItem(conn, m->SourType, m->SourPos, &pMob[conn].MOB.Carry[m->SourPos]);
...
#pragma endregion
	...
	{		
		...
		{
			SendClientMessage(conn, "No h Cinto equipado");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
	}
	if (item->sIndex == 5775)//Alicate Colar
	{
		...
		{
			SendClientMessage(conn, "No h Colares equipados");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
	}
	...
	if (item->sIndex >= 4500 && item->sIndex <= 4549) {
		...
		for (int i = 1; i < 16; i++) {
			...
			if (pUser[conn].Keys[i] == Traje) {
				SendClientMessage(conn, "Esse Traje j foi utilizado");
				SendItem(conn, m->SourType, m->SourPos, item);
				break;
			}
			if (i == 15 && pUser[conn].Keys[i] != 0) {
				SendClientMessage(conn, "No h slots de trajes disponveis");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
		}
