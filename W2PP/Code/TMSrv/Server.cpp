...
char Keywords1[30][7] = {
	"TREM", "BIKE", "CARRO", "BIKE", "AVIAOO",
	"TREM", "BIKE", "NAVIO", "MOTO", "BIKE",
	"CARRO", "MOTO", "NAVIO", "AVIAOO", "BIKE",
	"AVIAOO", "CARRO", "MOTO", "TREM", "MOTO",
	"BIKE", "AVIAOO", "BIKE", "NAVIO", "NAVIO",
	"AVIAOO", "NAVIO", "AVIAOO", "NAVIO", "NAVIO"
};

char Keywords2[30][7] = {
	"BIKE", "MOTO", "AVIAOO", "BIKE", "AVIAOO",
	"AVIAOO", "MOTO", "TREM", "NAVIO", "MOTO",
	"CARRO", "TREM", "CARRO", "BIKE", "CARRO",
	"NAVIO", "AVIAOO", "AVIAOO", "BIKE", "BIKE",
	"MOTO", "BIKE", "BIKE", "NAVIO", "MOTO",
	"NAVIO", "NAVIO", "AVIAOO", "TREM", "AVIAOO"
};
...
// Variáveis do sistema de ondas da Carta N
STRUCT_CARTA_N_INSTANCE CartaNInstances[MAX_CARTA_N_INSTANCES] = {0};

// Coordenadas de spawn para cada instância (centro definido no Basedef.h)
int CartaNInstancePos[MAX_CARTA_N_INSTANCES][2] =
{
	{ 808, 3666 },  // Instância 1 - centro (808, 3666)
	{ 862, 3677 },  // Instância 2 - centro da área (840-885, 3655-3700)
	{ 862, 3622 }   // Instância 3 - centro da área (840-885, 3600-3645)
};
...
void BASE_ReadQuestDiaria()
{
	...
	if (fp == NULL)
	{
		MessageBoxA(NULL, "NAOo foi possAOvel carregar o arquivo 'QuestDiaria.txt'", "TMSRV", MB_OK);
...
int Challange(int conn, int mob, int coin)
{
	...
	SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE `guilds` SET fame = '%d' WHERE `guild` = '%d'", GuildInfo[usGuild].Fame - 100, usGuild);
	pc.wQuery(xQuery);
...
int GenerateSummon(int conn, int SummonID, STRUCT_ITEM* sItem, int Num)
{
	...
	for (int i = SummonNum; i < Num; i++)
	{
		...
		if (SummonID >= 0 && SummonID <= 7)
			pMob[MobEmpty].IsSummon = 2; // EvocaAOAOes
	}
	return 1;
}
...
// ============================================================================
// FUNÇÕES DO SISTEMA DE ONDAS DA CARTA N
// ============================================================================

void GetInstanceArea(int instanceID, int* x1, int* y1, int* x2, int* y2)
{
	switch (instanceID)
	{
	case 0:
		*x1 = CARTA_N_INSTANCE1_X1; *y1 = CARTA_N_INSTANCE1_Y1;
		*x2 = CARTA_N_INSTANCE1_X2; *y2 = CARTA_N_INSTANCE1_Y2;
		break;
	case 1:
		*x1 = CARTA_N_INSTANCE2_X1; *y1 = CARTA_N_INSTANCE2_Y1;
		*x2 = CARTA_N_INSTANCE2_X2; *y2 = CARTA_N_INSTANCE2_Y2;
		break;
	case 2:
		*x1 = CARTA_N_INSTANCE3_X1; *y1 = CARTA_N_INSTANCE3_Y1;
		*x2 = CARTA_N_INSTANCE3_X2; *y2 = CARTA_N_INSTANCE3_Y2;
		break;
	default:
		*x1 = 0; *y1 = 0; *x2 = 0; *y2 = 0;
		break;
...
int GetCartaInstanceByPosition(int x, int y)
{
	// Instância 1: 774-815
	if (x >= CARTA_N_INSTANCE1_X1 && x <= CARTA_N_INSTANCE1_X2 &&
		y >= CARTA_N_INSTANCE1_Y1 && y <= CARTA_N_INSTANCE1_Y2)
		return 0;

	// Instância 2: 816-857
	if (x >= CARTA_N_INSTANCE2_X1 && x <= CARTA_N_INSTANCE2_X2 &&
		y >= CARTA_N_INSTANCE2_Y1 && y <= CARTA_N_INSTANCE2_Y2)
		return 1;

	// Instância 3: 858-892
	if (x >= CARTA_N_INSTANCE3_X1 && x <= CARTA_N_INSTANCE3_X2 &&
		y >= CARTA_N_INSTANCE3_Y1 && y <= CARTA_N_INSTANCE3_Y2)
		return 2;

	return -1;  // Não está em instância de Carta N
}

void ClearCartaInstanceMobs(int instanceID)
{
	int x1, y1, x2, y2;
	GetInstanceArea(instanceID, &x1, &y1, &x2, &y2);

	// Expandir área para garantir que todos os mobs sejam deletados (offset +/-15)
	for (int xx = x1 - 15; xx <= x2 + 15; xx++)
	{
		for (int yy = y1 - 15; yy <= y2 + 15; yy++)
		{
			if (xx < 0 || yy < 0 || xx >= MAX_GRIDX || yy >= MAX_GRIDY)
				continue;
...
void SpawnCartaNWave(int instanceID)
{
	STRUCT_CARTA_N_INSTANCE* pInstance = &CartaNInstances[instanceID];
	int wave = pInstance->CurrentWave;

	// Limpar mobs anteriores da instância
	ClearCartaInstanceMobs(instanceID);

	// Resetar CurrentNumMob dos mobs da Carta N para garantir spawn correto
	for (int mobID = CARTA_N_WAVE_MOB_START; mobID <= CARTA_N_WAVE_MOB_END; mobID++)
	{
		mNPCGen.pList[mobID].CurrentNumMob = 0;
	}

	// Posição base da instância (mesma posição onde o jogador spawna)
	int baseX = CartaNInstancePos[instanceID][0];
	int baseY = CartaNInstancePos[instanceID][1];

	// Selecionar mobs baseados na instância
	int wave1Mob, wave2Mob, wave3Mob, bossMob;

	switch (instanceID)
	{
	case 0: // Instância 1 - Carta N mobs
		wave1Mob = CARTA_N_I1_WAVE1_MOB;  // Meio_Orc_ (2391)
		wave2Mob = CARTA_N_I1_WAVE2_MOB;  // Caveira_Negra@ (2393)
		wave3Mob = CARTA_N_I1_WAVE3_MOB;  // Caveira_Lanc_ (2395)
		bossMob = CARTA_N_I1_BOSS;         // Troll_Caos___ (2399)
		break;
	case 1: // Instância 2 - Carta M mobs
		wave1Mob = CARTA_N_I2_WAVE1_MOB;  // Elfo_Negro__ (2401)
		wave2Mob = CARTA_N_I2_WAVE2_MOB;  // Rei_Taurus__ (2403)
		wave3Mob = CARTA_N_I2_WAVE3_MOB;  // Hidra_ (2405)
		bossMob = CARTA_N_I2_BOSS;         // Zakum__ (2409)
		break;
	case 2: // Instância 3 - Carta A mobs
		wave1Mob = CARTA_N_I3_WAVE1_MOB;  // Elfo_Negro_Abj_ (2411)
		wave2Mob = CARTA_N_I3_WAVE2_MOB;  // Tauron_____ (2413)
		wave3Mob = CARTA_N_I3_WAVE3_MOB;  // Morlock_ (2415)
		bossMob = CARTA_N_I3_BOSS;         // Lugefer__ (2419)
		break;
	default:
		wave1Mob = CARTA_N_I1_WAVE1_MOB;
		wave2Mob = CARTA_N_I1_WAVE2_MOB;
		wave3Mob = CARTA_N_I1_WAVE3_MOB;
		bossMob = CARTA_N_I1_BOSS;
		break;
	}
	...
	if (wave == 1)
	{
		...
		for (int i = 0; i < 50; i++)
		{
			GenerateMob(wave1Mob, baseX, baseY);
		}
		pInstance->MobsTotal = 50;
	}
	else if (wave == 2)
	{
		// Onda 2: 50 mobs
		for (int i = 0; i < 50; i++)
		{
			GenerateMob(wave2Mob, baseX, baseY);
		}
		pInstance->MobsTotal = 50;
	}
	else if (wave == 3)
	{
		...
		for (int i = 0; i < 50; i++)
		{
			GenerateMob(wave3Mob, baseX, baseY);
		}
		pInstance->MobsTotal = 50;
	}
	else if (wave == 4)
	{
		// Onda 4: BOSS
		GenerateMob(bossMob, baseX, baseY);
		pInstance->MobsTotal = 1;
	}

	pInstance->MobsKilled = 0;
	pInstance->WaveTime = 60;  // 1 minuto para proxima onda (se nao matar todos)
	SendWaveInfo(instanceID);  // Enviar contador ao grupo
}

void CompleteCartaNInstance(int instanceID)
{
	STRUCT_CARTA_N_INSTANCE* pInstance = &CartaNInstances[instanceID];

	// Dar drops do boss
	CartaDrop(pInstance->LeaderConn, pInstance->LeaderConn, 4);  // conn, target, type 4 = boss drops

	// Teleportar líder primeiro
	int leaderConn = pInstance->LeaderConn;
	if (leaderConn > 0 && leaderConn < MAX_USER && pUser[leaderConn].Mode == USER_PLAY)
	{
		DoTeleport(leaderConn, 2100, 2100);  // Cidade principal - área segura
		SendClientMessage(leaderConn, "Carta N completada! Voce voltou para a cidade.");
	}

	// Teleportar resto do grupo
	for (int i = 0; i < MAX_PARTY; i++)
	{
		int partyconn = pMob[leaderConn].PartyList[i];
		if (partyconn > 0 && partyconn < MAX_USER && partyconn != leaderConn && pUser[partyconn].Mode == USER_PLAY)
		{
			DoTeleport(partyconn, 2100, 2100);  // Cidade principal - área segura
			SendClientMessage(partyconn, "Carta N completada! Voce voltou para a cidade.");
		}
	}

	// Limpar instância
	memset(&CartaNInstances[instanceID], 0, sizeof(STRUCT_CARTA_N_INSTANCE));
}
...
BOOL WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!ret)
	{
		//Log("erro,nAOo foi possivel iniciar a seguinte funAOAOo : WSAInitialize", "-system", 0);
	...
	if (fp)
	{
		...
		MessageBox(hWndMain, "NAOo foi possivel encontrar o arquivo LocalIP.txt", "Erro ao iniciar", NULL);
	...
	if (DBServerAddress[0] == 0)
	{
		MessageBox(hWndMain, "NAOo foi possivel pegar o ServerGroup. LocalIP.txt / ServerList.txt", "Erro ao iniciar", MB_OK | MB_SYSTEMMODAL);
	...
	if (ret == NULL)
	{
		Log("erro, NAOo foi possivel se conectar a DBServer", "-system", 0);
		MessageBox(hWndMain, "NAOo foi possivel se conectar a DBServer", "Erro ao iniciar", NULL);
	...
	if (BILLING > 0)
	{
		...
		if (ret == NULL)
		{
			Log("erro, NAOo foi possivel se conectar na BIServer", "-system", 0);
			BILLING = 0;
		}
		else
		{
...
#pragma region Bloqueio de excesso de tentativas por IP
			int DEFAULT_BLOCKIP = 5; // consideramos 3 segundos aqui, agora AO pra ser 2 minutos
			TMP_BLOCKIP bIp = TMP_BLOCKIP(pUser[User].IP, DEFAULT_BLOCKIP);

			// verificamos se o usuAOrio jAO estAO na lista, se ele estiver, a conexAOo serAO anulada e nada serAO processado pela dbsrv ou tmsrv.
			...
			// adiciona o IP do usuAOrio na lista para evitar com que requisite mais de uma conexAOo em um curto periodo de tempo
...
#pragma endregion
	...
	case WM_CLOSE:
	{
		if (MessageBox(hWndMain, "VocAO realmente deseja desligar o server?", "Aviso!!!", MB_ICONQUESTION | MB_YESNO) == IDNO)
			break;
...
void SendItemagrupar(const int client, int itemID)
{
	...
	//Verificar espaAOo no inventAOrio
	...
	if (invfree < 1)
	{
		...
		SendClientMessage(client, "Seu inventAOrio estAO cheio");
...
// ========================================================================
// FUNÇÃO SendItemagruparEx - Agrupa itens corretamente com quantidade
// Criada para corrigir bug onde itens com quantidade > 1 não agrupavam
// ========================================================================
void SendItemagruparEx(const int client, int itemID, int amount)
{
	if (client <= 0 || client >= MAX_USER)
		return;
	if (pUser[client].Mode != USER_PLAY)
		return;
	if (itemID <= 0 || amount <= 0)
		return;

	// Primeiro: procurar se já existe o item agrupado no inventário
	for (int i = 0; i < pMob[client].MaxCarry; i++)
	{
		if (pMob[client].MOB.Carry[i].sIndex == itemID)
		{
			// Verificar se este slot tem efeito 61 (agrupável)
			if (pMob[client].MOB.Carry[i].stEffect[0].cEffect == 61 ||
				pMob[client].MOB.Carry[i].stEffect[1].cEffect == 61 ||
				pMob[client].MOB.Carry[i].stEffect[2].cEffect == 61)
			{
				int currentAmount = BASE_GetItemAmount(&pMob[client].MOB.Carry[i]);

				// Verificar limite máximo
				if (currentAmount >= 120)
					continue;  // Tentar próximo slot

				int newAmount = currentAmount + amount;
				if (newAmount > 120)
					newAmount = 120;

				BASE_SetItemAmount(&pMob[client].MOB.Carry[i], newAmount);
				SendItem(client, ITEM_PLACE_CARRY, i, &pMob[client].MOB.Carry[i]);
				return;  // Sucesso!
			}
		}
	...
	// Segundo: não encontrou item existente - criar novo em slot vazio
	for (int i = 0; i < pMob[client].MaxCarry; i++)
	{
		if (pMob[client].MOB.Carry[i].sIndex == 0)
		{
			STRUCT_ITEM item;
			memset(&item, 0, sizeof(STRUCT_ITEM));
			item.sIndex = itemID;
			item.stEffect[0].cEffect = 61;
			item.stEffect[0].cValue = amount > 120 ? 120 : amount;

			memcpy(&pMob[client].MOB.Carry[i], &item, sizeof(STRUCT_ITEM));
			SendItem(client, ITEM_PLACE_CARRY, i, &pMob[client].MOB.Carry[i]);
			return;  // Sucesso!
		}
	}
...
void DecideWinner0()
{
	...
	// VerificaAOAOo para setar Noatum
...
//		GuildZoneReport();
//	}
//	//A guerra de Nipplehein irAO comeAOar em 3 minutos e Teleporta os Players (20:17)
//	if (WeekMode == 1 && timeinfo->tm_wday == 0 && timeinfo->tm_hour == GuildHour && timeinfo->tm_min == 17)
//	{
//		SendNotice("A Guerra de Nipplehein irAO comeAOar em 3 minutos");
//		ClearGuildPKZone();
//		WeekMode = 2;
//		SetArenaDoor(STATE_LOCKED);
//
...
//		GuildZoneReport();
//	}
//	//A guerra de Azran irAO comeAOar em 3 minutos e Teleporta os Players (20:32)
//	if (WeekMode == 1 && timeinfo->tm_wday == 0 && timeinfo->tm_hour == GuildHour && timeinfo->tm_min == 32)
//	{
//		SendNotice("A Guerra de Azran irAO comeAOar em 3 minutos");
//		ClearGuildPKZone();
//		WeekMode = 2;
//		SetArenaDoor(STATE_LOCKED);
//
...
//		GuildZoneReport();
//	}
//	//A guerra de Armia irAO comeAOar em 3 minutos e Teleporta os Players (20:47)
//	if (WeekMode == 1 && timeinfo->tm_wday == 0 && timeinfo->tm_hour == GuildHour && timeinfo->tm_min == 47)
//	{
//		SendNotice("A Guerra de Armia irAO comeAOar em 3 minutos");
//		ClearGuildPKZone();
//		WeekMode = 2;
//		SetArenaDoor(STATE_LOCKED);
//
		...
		if (RvRState == 3 && timeinfo->tm_min == 34)
		{
			for (int conn = 1; conn < MAX_USER; conn++)
			{
				if (pMob[conn].MOB.Clan)
				{
					...
					{
						SendNotice("Guerra de Reinos terminou em empate [Sem BAOnus RVR]. ");
						RvRBonus = 0;
					}
				}
			}
...
#pragma endregion*/
	...
	SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE accounts SET online = '%d' WHERE username = '%s' ", 0, escapedAccount);
	pc.wQuery(xQuery);
		...
		else
		{
			...
			if (conn == Desafiantes[0]) {
				...
				if (pUser[Desafiantes[1]].Mode == USER_PLAY) {
					...
					SendClientMessage(Desafiantes[1], "!VocAO recebeu sua barrra de Bi de volta");
					DoTeleport(Desafiantes[1], 2100, 2100);

					PutItem(Desafiantes[1], &I);
					SendClientMessage(Desafiantes[1], "!VocAO venceu por W.O.");
				}
			...
			if (conn == Desafiantes[1]) {
				...
				if (pUser[Desafiantes[0]].Mode == USER_PLAY) {
					...
					SendClientMessage(Desafiantes[0], "!VocAO recebeu sua barrra de Bi de volta");
					DoTeleport(Desafiantes[0], 2100, 2100);

					PutItem(Desafiantes[0], &I);
					SendClientMessage(Desafiantes[0], "!VocAO venceu por W.O.");
				}
...
void ReadLevelItemConfig(void)
{
	...
	while (1)
	{
		...
		if (cls == 4 && type != 4)//Item para todas as classes mais nAOo para todas build
		{
...
void DoItemLevel(int conn)
{
	...
	if (LevelItem[pMob[conn].MOB.Class][type][pMob[conn].MOB.BaseScore.Level].sIndex != 0)
	{
		for (int i = 0; i < MAX_CARGO - 2; i++)
		{
			...
			strncat(temp, ItemCode, sizeof(temp) - strlen(temp) - 1);
...
/*void SetCircletSubGod(int conn)
{
	...
	{
		if (pMob[conn].MOB.BaseScore.Level == 119 && pMob[conn].extra.QuestInfo.Celestial.Add120 == 0) // NAOvel 120
		{
			if (type1)
			{
		...
		if (pMob[conn].MOB.BaseScore.Level == 149 && pMob[conn].extra.QuestInfo.Celestial.Add150 == 0) // NAOvel 150
		{
			if (type1)
			{
				pMob[conn].MOB.Equip[1].stEffect[0].cValue = typefv1;
		...
		if (pMob[conn].MOB.BaseScore.Level == 179 && pMob[conn].extra.QuestInfo.Celestial.Add180 == 0) // NAOvel 180
		{
			if (type1)
			{
				pMob[conn].MOB.Equip[1].stEffect[0].cValue = typefv1;
		...
		if (pMob[conn].MOB.BaseScore.Level == 199 && pMob[conn].extra.QuestInfo.Celestial.Add200 == 0) // NAOvel 200
		{
			if (type1)
			{
				pMob[conn].MOB.Equip[1].stEffect[0].cValue = typefv1;
...
{
	if (Item->sIndex < 0 || Item->sIndex > 6500) // Id invAOlido
		return false;
	else if (ef1 && !efv1) // Diz que vai por add mas nAOo tem o valor
		return false;
	else if (ef2 && !efv2) // '''''''''''''''''''''''''''''''''''''''
		return false;
	else if (!ef1 && efv1) // '''''''''''''''''''''''''''''''''''''''
	...
	if (Sanc)
	{
		...
		// PAOe a sanc anterior do item nele novamente
...
void ReadDirectory()
{
	...
	{
		MessageBox(hWndMain, "NAOo foi possivel abrir Account_Directory.txt!", "Erro ao iniciar", NULL);
		return;
	}
	else
	{
