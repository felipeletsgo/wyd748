...
void Exec_MSG_CombineItem(int conn, char* pMsg)
{
	...
	{
		SendClientMessage(conn, "Imposs�vel executar a��o11, tente mais tarde. ");
		return;
	}
	...
	if (_rand <= combine || LOCALSERVER)
	{
		...
		if (extra > 0 && extra < MAX_ITEMLIST)
		{
			...
			if (joia >= 0 && joia <= 3)
			{
				...
				// Calcular porcentagem real de sucesso
				int chancePercent = combine;
				if (chancePercent > 100) chancePercent = 100;

				char msgSucesso[128];
				sprintf(msgSucesso, "SUCESSO NA COMPOSICAO ANCT (%d/%d)", _rand, chancePercent);
				SendClientMessage(conn, msgSucesso);
	...
	{
		// Mensagem de falha melhorada
		int chancePercent = combine;
		if (chancePercent > 100) chancePercent = 100;

		char msgFalha[128];
		sprintf(msgFalha, "FALHA NA COMPOSICAO ANCT (%d/%d)", _rand, chancePercent);
		SendClientMessage(conn, msgFalha);

		char logMsg[256];
		sprintf(logMsg, "Falha na composicao anct - rolagem %d/%d", _rand, chancePercent);
		Combinelogsitem(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, logMsg);
