...
void Exec_MSG_ReqBuy(int conn, char *pMsg)
{
	...
	if (itemPrice == 1) {
		SendClientMessage(conn, "Itens com custo 1 de Gold são apenas demonstrativos");
		return;
	}
	...
	if (targetID == conn) {
		SendClientMessage(conn, "Não pode comprar de si mesmo");
		return;
	}
	...
	int MobId = targetID + 25000;

	if (pMob[conn].TargetX >= pMob[MobId].TargetX - VIEWGRIDX && pMob[conn].TargetX <= pMob[MobId].TargetX + VIEWGRIDX && pMob[conn].TargetY >= pMob[MobId].TargetY - VIEWGRIDY && pMob[conn].TargetY <= pMob[MobId].TargetY + VIEWGRIDY)
	{
		...
		int target_village = BASE_GetVillage(pMob[MobId].TargetX, pMob[MobId].TargetY);
