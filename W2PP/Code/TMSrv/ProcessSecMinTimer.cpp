...
#pragma region Sistema de bloqueio de ip por requisies de conexo.
...
#pragma endregion
		...
		// ========================================================================
		// TIMER DE ONDAS DA CARTA N (Sistema de Instâncias)
		// ========================================================================
		for (int i = 0; i < MAX_CARTA_N_INSTANCES; i++)
		{
			if (CartaNInstances[i].Active)
			{
				// Verificar se o líder ainda está online e na área da instância
				int leaderConn = CartaNInstances[i].LeaderConn;
				if (leaderConn <= 0 || leaderConn >= MAX_USER || pUser[leaderConn].Mode != USER_PLAY)
				{
					// Líder desconectou - liberar instância
					ClearCartaInstanceMobs(i);
					memset(&CartaNInstances[i], 0, sizeof(STRUCT_CARTA_N_INSTANCE));
					continue;
				}

				// Verificar se o líder ainda está na área da instância
				int leaderX = pMob[leaderConn].TargetX;
				int leaderY = pMob[leaderConn].TargetY;
				int leaderInstance = GetCartaInstanceByPosition(leaderX, leaderY);

				if (leaderInstance != i)
				{
					// Líder saiu da área da instância - liberar instância
					ClearCartaInstanceMobs(i);
					memset(&CartaNInstances[i], 0, sizeof(STRUCT_CARTA_N_INSTANCE));
					continue;
				}

				// Timer só funciona durante as ondas 1-3 (não no boss)
				if (CartaNInstances[i].CurrentWave < 4)
				{
					if (CartaNInstances[i].WaveTime > 0)
					{
						CartaNInstances[i].WaveTime--;

						// Atualizar timer no cliente a cada segundo
						for (int p = 0; p < MAX_PARTY; p++)
						{
							int partyconn = pMob[CartaNInstances[i].LeaderConn].PartyList[p];
							if (partyconn > 0 && partyconn < MAX_USER && pUser[partyconn].Mode == USER_PLAY)
							{
								SendClientSignalParm(partyconn, ESCENE_FIELD, _MSG_StartTime, CartaNInstances[i].WaveTime);
							}
						}
					}

					// Tempo expirou - próxima onda (fallback se mobs nao morreram)
					if (CartaNInstances[i].WaveTime == 0)
					{
						CartaNInstances[i].CurrentWave++;
						SpawnCartaNWave(i);
						...
						for (int p = 0; p < MAX_PARTY; p++)
						{
							...
							{
								char msg[128];
								sprintf(msg, "Tempo esgotado! Onda %d iniciada!", CartaNInstances[i].CurrentWave);
		...
		// ========================================================================
		// FIM DO TIMER DE ONDAS DA CARTA N
		// ========================================================================

#pragma region Pesadelo Verification
...
#pragma region >> Verifica se o usurio est com vrias contas abertas
		...
		if (ApostaTimer > 0) {
			...
			if (ApostaTimer == 0 && Desafiantes[0] != 0 && Desafiantes [1] == 0) {
				...
				if (pUser[Desafiantes[0]].Mode == USER_PLAY) {
					...
					SendClientMessage(Desafiantes[0], "!Voc recebeu sua barrra de Bi de volta");
				}				
				Desafiantes[0] = 0;
			}
			if (ApostaTimer == 6 && Desafiantes[0] != 0 && Desafiantes[1] != 0) {
				...
				if (pUser[Desafiantes[1]].Mode == USER_PLAY) {
					...
					SendClientMessage(Desafiantes[1], "!Voc recebeu sua barrra de Bi de volta");
				...
				if (Outro == 1) {
					PutItem(Desafiantes[0], &I);
					SendClientMessage(Desafiantes[0], "!Voc venceu por W.O.");
				}

				if (Outro == 3) {
					PutItem(Desafiantes[1], &I);
					SendClientMessage(Desafiantes[1], "!Voc venceu por W.O.");
				}
...
#pragma region Boss Time
	if (SecCounter % 2 == 0)
	{
		...
		{
			SendNoticeArea("Os Cavaleiros Esqueleto esto atacando a estrutura central!", 1664, 3584, 1919, 3711);
		...
		if (SecCounter % 2 == 0)
		{
			for (int i = 1; i < MAX_USER; i++)
			{
				...
				if (pUser[i].Ingame.Lojinha == 1 && pMob[i].CheckLojinha <= 0)
				{
					...
					bPassMission(i, 4);
					SendClientMessage(i, "Voc Recebeu sua recompensa");
					SendEtc(i);
				}
...
#pragma region Controle de Drop por Tempo
		...
		{
			ItemDropControl[671] = 1; //Fora Espectral
			ItemDropControl[1741] = 1; //Alma da Fnix
		}

#pragma region >> Resete Pistas precauo
			...
			if (when.tm_hour == 20)
			{
				if ((when.tm_min == 15) && when.tm_sec == 0)
				{
					if (SecCounter % 2 == 0)
					{
						...
						SendNoticeArea(" O Baro Acabou de Renascer !!!", 1053, 1949, 1246, 2148);
...
#pragma region BOSS GLOBAL
		if (((when.tm_wday != 0 && when.tm_wday != 6) && (when.tm_hour == 0 || when.tm_hour == 4 || when.tm_hour == 8 || when.tm_hour == 12 || when.tm_hour == 16 || when.tm_hour == 20) && when.tm_min == 0 && when.tm_sec <= 3) && g_BSombraNegra == FALSE) // HORARIO DO BOSS - Segunda  Sexta de 4 em 4 horas
		{
			GenerateMob(SOMBRANEGRA, 0, 0);
			SendNotice("Sombra Negra Nasceu!");
			g_BSombraNegra = TRUE;
		}
		//if (((when.tm_wday != 0 && when.tm_wday != 6) && when.tm_hour == 18 && when.tm_min == 40 && when.tm_sec <= 3) && g_BVerid == FALSE) // HORARIO DO BOSS - Segunda  Sexta 18:40 da noite
		//{
		//	GenerateMob(VERIDNEGRO, 0, 0);
		//	SendNotice("O Verid Negro acabou de renascer!");
		//	g_BVerid = TRUE;
		//}
		if (((when.tm_wday != 0 && when.tm_wday != 6) && when.tm_hour == 19 && when.tm_min == 40 && when.tm_sec <= 3) && g_RainhaG == FALSE) // HORARIO DO BOSS - Segunda  Sexta 19:40 da noite		
		{
...
#pragma endregion
		...
		//			}
		//
		//			SendNotice("Ningum foi confrontar a Rainha");
		//		}
		//#pragma endregion
...
#pragma region Porto Reset
		if ((when.tm_min == 29 || when.tm_min == 59) && when.tm_sec == 0)
		{
			DeleteMobMapa(13, 28);
			//ClearMapa(13, 28);//Porto Infernal
...
//					SendNotice("Batalha Real Finalizada! - No houve ganhadores");
//					BrState = 0;
//					BrTime = 0;
//				}
//			}
...
#pragma endregion
#pragma region Pista de runas sada
...
#pragma region Avisos
		...
		{
			SendNotice("Voc  NOVATO por aqui ? Digite /novato");
			return;
		}*/
...
//#pragma region Ataque / fala em ao dos mobs
...
#pragma region Limpa as propriedades do mob caso no foram removidas pelo mtodo padro
if (SecCounter % 30 == 0)
{
...
void ProcessMinTimer()
{
	...
	for (int i = 0; i < MAX_GUILDZONE; i++)
	{
		...
		STRNCAT_SAFE(temp, name, sizeof(temp));
