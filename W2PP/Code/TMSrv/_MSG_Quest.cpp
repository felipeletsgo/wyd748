...
void Exec_MSG_Quest(int conn, char* pMsg)
{
	...
	{
		SendClientMessage(conn, "Imposs�vel executar a��o36, tente mais tarde. ");
		return;
	}
...
#pragma endregion
	...
	case APOSTA:
	{
		if (Desafiantes[0] != 0 && Desafiantes[1] != 0) {
			SendSay(conn, "Uma aposta est� em andamento");
			return;
		}
		if (Desafiantes[0] != 0 && Desafiantes[1] == 0) {
			if (RemoveItem(conn, 4011, 1) == true) {
				...
				SendNotice(strFmt("O duelo entre %s e %s come�ou!", pMob[Desafiantes[0]].MOB.MobName, pMob[conn].MOB.MobName));
				ApostaTimer = 11;
			}
			else {
				SendSay(conn, "Voc� precisa de [1] Barra de 1 Bi");
			}
			return;
		}
		if (Desafiantes[0] == 0) {
			if (RemoveItem(conn, 4011, 1) == true) {
				Desafiantes[0] = conn;	
				ApostaTimer = 5;
				SendNotice(strFmt("O Jogador %s deu inicio � aposta PvP em Noatum!", pMob[conn].MOB.MobName));
				SendClientMessage(conn, "!O desafio ser� cancelado e voc� receber� seu dinheiro de volta");
				SendClientMessage(conn, "!Em aproximadamente 5 minutos se n�o houver desafiante");
			}
			else {
				SendSay(conn, "Voc� precisa de [1] Barra de 1 Bi");
			}
...
#pragma region KING
	...
	/*	if (TradingCape == TRUE)
		...
		if (pMob[conn].MOB.Equip[10].sIndex == 1742 && pMob[conn].MOB.Equip[11].sIndex >= 1760 && pMob[conn].MOB.Equip[11].sIndex <= 1763)
		{
			...
			if (pMob[conn].extra.ClassMaster == MORTAL && pMob[conn].MOB.CurrentScore.Level >= 380 && pMob[conn].Mode != MOB_WAITDB)
			{
				if (EvolutionStatus < 1) {
					SendClientMessage(conn, "Cria��o de Arch Bloqueada");
					return;
				}
		...
		if ((pMob[conn].extra.ClassMaster == CELESTIAL || pMob[conn].extra.ClassMaster == CELESTIALCS || pMob[conn].extra.ClassMaster == SCELESTIAL || pMob[conn].extra.ClassMaster == HARDCORE) && (pMob[conn].MOB.Equip[15].sIndex == 3199 || pMob[conn].MOB.Equip[15].sIndex == 573))
		{
			...
			if (SlotId == -1) {
				SendClientMessage(conn, "Precisa haver espa�o disponivel no invent�rio");
				return;
			}
...
#pragma region UXMAL
	case UXMAL:
	{
		...
		{
			SendClientMessage(conn, "Entrada J� esta registrada");
			break;
		}*/
...
#pragma region GOLD_DRAGON
	case GOLD_DRAGON:
	{
		...
		if (Cristais[0] != -1 && Cristais[1] != -1 && Cristais[2] != -1 && Cristais[3] != -1 && Cristais[4] != -1 && Cristais[5] != -1 && Cristais[6] != -1)
		{
			...
			{
				SendSay(npcIndex, "Voc� precisa equipar a arma desejada");
				break;
			}
			...
			if (sanc != 9)
			{
				SendSay(npcIndex, "Apenas armas +9 poder�o ser modificadas");
				break;
			}

			if (pMob[conn].MOB.Coin < 50000000) {
				SendClientMessage(conn, "Voc� precisa pagar 50kk para modificar os atributos");
				return;
			}
			...
			if (Level >= 6) {
				SendSay(npcIndex, "Apenas armas n�o Anct podem ser modificadas");
				break;
			}
			...
			SendClientMessage(conn, "Voc� precisa ter os 7 Cristais + 50kk para melhorar sua arma");
		}
...
#pragma region SOBREVIVENTE
	case SOBREVIVENTE:
	{
		...
		if (pMob[conn].extra.KefraTicket < 1) {
			...
			SendClientMessage(conn, "Voc� n�o possui entradas!");
			...
			if (i != pMob[conn].MaxCarry)
			{
				...
				SNPRINTF_SAFE(temp, sizeof(temp), "etc,kefraticket ticket:%d name:%s conn:%d", pMob[conn].extra.KefraTicket, pMob[conn].MOB.MobName, conn);
				Log(temp, 0, pUser[conn].IP);

				SNPRINTF_SAFE(temp, sizeof(temp), g_pMessageStringTable[_DN_CHANGE_COUNT], pMob[conn].extra.KefraTicket);
...
#pragma endregion
	case FRAGMENTODAALMA:
	{
		...
		if (invfree <= 0) //s�o 2 itens q recebe (coloca 0 no code), pq 1 � do pr�prio ba�
		{
			SendClientMessage(conn, "Falta espa�o no invent�rio.");
			return;
		}
...
#pragma region KRUNO
	case KRUNO:
	{
		...
		int ItemReq = 4123; //PEDA�O DE CHANCE
...
#pragma region QUEST_EQUILIBRIO
	case QUEST_EQUILIBRIO:
	{
		...
		SendSay(npcIndex, "Que voc� consiga sua salva��o.");
...
#pragma region AMELIA
	case AMELIA:
	{
		...
		int ItemReq = 4125; //PEDA�O DE EQUILIBIRO
...
#pragma region JULIARZAN
	case JULIARZAN:
	{
		...
		{
			SendSay(npcIndex, "Voc� precisa de 50,000 mil");
			break;
		}
...
#pragma region JULIGELO
	case JULIGELO:
	{
		...
		if (pMob[conn].MOB.Coin < 50000)
		{
			SendSay(npcIndex, "Voc� precisa de 50,000 mil de gold");
			break;
		}
...
#pragma region UNICORNIO_PURO
	case UNICORNIO_PURO:
	{
		if (pMob[conn].MOB.Equip[6].sIndex == 862)//espada curta
		{
			...
			SendSay(npcIndex, "Tome este presente, mas n�o se acostume a isso...");
			SendItem(conn, ITEM_PLACE_EQUIP, 6, &pMob[conn].MOB.Equip[6]);
			return;
		}
		...
		if (pMob[conn].MOB.Equip[6].sIndex == 918)//LAN�A MAGICA OK
		{
