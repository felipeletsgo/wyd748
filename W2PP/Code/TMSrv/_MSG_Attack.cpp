...
void Exec_MSG_Attack(int conn, char* pMsg)
{
	...
	{
		SendClientMessage(conn, "Imposs�vel executar a��o6, tente mais tarde.");
		return;
	}
	...
	//Personagem morto tentando usar uma skill que n�o � ressurei��o
...
#pragma region Consumo de mana
	if (skillnum >= 0 && skillnum < MAX_SKILLINDEX)
	{
		...
		if (pMob[conn].MOB.Class == 0 && (pMob[conn].extra.SecLearnedSkill & 0x100)) // Espelho M�gico
		{
			pMob[conn].MOB.CurrentScore.Mp = pMob[conn].MOB.CurrentScore.Mp - ManaSpent / 2;
			pUser[conn].ReqMp = pUser[conn].ReqMp - ManaSpent / 2;
			SetReqMp(conn);
...
#pragma endregion
			...
			//if ((pMob[conn].MOB.Equip[0].sIndex == 11 || pMob[conn].MOB.Equip[0].sIndex == 16 || pMob[conn].MOB.Equip[0].sIndex == 17 || pMob[conn].MOB.Equip[0].sIndex == 18 || pMob[conn].MOB.Equip[0].sIndex == 19) && pMob[conn].MOB.LearnedSkill & 15)// colocar todos os index de class FM, s�o 4 verificar se 15 � a oitava de fm
			//	dam -= (dam * 0) / 100;// Aqui estamos diminuindo 40% do seu dano das skills, caso queira mudar, � s� mudar o valor ali de 40 pra outro valor
			...
			if (skillnum == 86) //Explos�o Et�ria
			{
...
#pragma endregion
			...
			//				{
			//					SendClientMessage(conn, "Voc� n�o pode usar essa skill em membros que n�o forem da sua guild ou estejam em Grupo.");
			//					return;
			//				}
			//
			//				int hp = (rand() % 10 + 90) * ((pMob[conn].MOB.CurrentScore.MaxHp + 1) / 100);
...
#pragma endregion
		...
		if (dam <= 0)
		{
			...
			{
				if (pMob[conn].Affect[a].Type == 30 && pMob[conn].Affect[a].Level >= 2000)
				{
					FrangoDamage = 2000;
					break;
				}
...
#pragma region PK - War - Miss
		...
		if (Summoner < MAX_USER)
		{
			...
			if (idx != conn)
			{
				...
				if (pMob[conn].extra.SecLearnedSkill & 0x100) // Espelho M�gico
				{
				...
				if (dam == -4 || dam == -3)  // Verifica se o DAM � -4 ou -3;
					pMob[idx].MissPlayer = TRUE; // Define na variavel MissPlayer quando o idx ataca ele � o conn, definindo o conn eu como idx;

				if (pMob[conn].MissPlayer == TRUE && dam > 0) // Verifica se o dano da variavel MissPlayer � -4 ou -3 e compara 
				{
...
#pragma endregion
		...
		if (pMob[idx].MOB.Class == 0) // Prote��o Divina
		{
			for (int i = 0; i < MAX_AFFECT; i++)
			{
		...
		if (pMob[idx].MOB.Class == 1) // Prote��o Absoluta
		{
			for (int i = 0; i < MAX_AFFECT; i++)
			{
				if (pMob[idx].Affect[i].Type == 6 && dam > 0)
...
#pragma region Controle de Mana
		if (pMob[idx].MOB.Class == 1) {
			for (int c = 0; c < MAX_AFFECT; c++) // Controle de Mana ( PVP )
			{
				...
				{
					// Reserva de 5% de mana para n�o tomar dano na mana e n�o conseguir usar skill
