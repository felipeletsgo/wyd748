...
void BalanceioPvP()
{
	...
	SNPRINTF_SAFE(xQuery, sizeof(xQuery), "SELECT * FROM `balanceamento` LIMIT 50");
	MYSQL_ROW row;
...
int DamageBalancing(int conn, int idx, int tDamage)
{
	...
	if (idx > 0 && idx < MAX_USER)
	{
		...
		int Evolucao = pMob[conn].extra.ClassMaster;
...
void SendBanAccount(int conn, int type)
{
	...
	case Banned::Tempo3horas:
	{
		...
		SNPRINTF_SAFE(temp, sizeof(temp), "!Conta [%s] bloqueada at� as: [%02d:%02d]  de [%02d/%02d/%04d]", pUser[conn].AccountName, ban->hora, ban->min, ban->dia, ban->mes + 1, ban->ano + 1900);
		SNPRINTF_SAFE(temp, sizeof(temp), "Nome da conta: [%s] Data de Banimento: [%02d:%02d] de [%02d/%02d/%04d] Motivo: Grade B", pUser[conn].AccountName, ban->hora, ban->min, ban->dia, ban->mes + 1, ban->ano + 1900);
...
SNPRINTF_SAFE(temp, sizeof(temp), "err,gridmulticast add %d-%d %s", tmob, pUser[tmob].Mode, pUser[tmob].AccountName);
Log(temp, "-system", 0);
pMobGrid[i][k] = 0;
CloseUser(tmob);
}
...
pUser[tmob].AccountName[ACCOUNTNAME_LENGTH - 1] = 0;
pUser[tmob].AccountName[ACCOUNTNAME_LENGTH - 2] = 0;
SNPRINTF_SAFE(temp, sizeof(temp), "err,grid-1 empty %d-%d %s", tmob, pUser[tmob].Mode, pUser[tmob].AccountName);
Log(temp, "-system", 0);
pMobGrid[i][k] = 0;
CloseUser(tmob);
}
...
//						}
//						if (Segment == 3) 
//							SendMsgExp(tmob, "[EXP 3/4]", TNColor::GoldenEscuro, false); // DESCRI��O XP PINHEIRO
//
//						if (Segment == 2) 
...
//							if (PunicaoCP > 75) //remove puni�ao cp
//							{
...
//							}
//
//							if (PunicaoCP > 30  && PunicaoCP <= 74) //remove puni�ao cp
//							{
		...
		for (int nX = minPosX; nX < maxPosX; nX++)
		{
			...
			if (sendPak->Type == _MSG_CNFMobKill)
			{
				...
				if (Segment >= 1 && Segment <= 4)
				{
					...
					if (Segment == 3)
						SendMsgExp(mobID, "[EXP 3/4]", TNColor::GoldenEscuro, false); // DESCRI��O XP PINHEIRO
...
void SendGuildList(int conn)
{
	...
	for (int i = 0; i < MAX_USER; i++)
	{
		...
		strncat(str, pMob[i].MOB.MobName, sizeof(str) - strlen(str) - 1);
...
void SendLanCount(int conn)
{
	if (g_LanNBoss == FALSE) {
		if (g_LanNBossCount >= 1000) //EDITAR QUANTIDADE DE MOB QUE PRECISA MATAR NA LAN PARA NACER O BOSS
		{
			...
			SendNoticeArea("*** O Guardi�o do Territ�rio Apareceu ***", 3604, 3601, 3690, 3690); //AVISO QUE BOSS NACEU PARA JOGADORES
			g_LanNBoss = TRUE;
			g_LanNBossCount = 0;
			return;
		}
...
void SendEventCount2(int conn)
{
	...
	if (premium == false) {
		if (wUser[conn].CounterHist > 360)
		{
			...
			if (cont >= 500)
				SendClientMessage(conn, "Limite m�ximo de recompensa alcan�ado");
...
// ============================================================================
// FUNÇÕES DE UI DO SISTEMA DE ONDAS DA CARTA N
// ============================================================================

void SendMobCounter(int instanceID, int remaining)
{
	if (instanceID < 0 || instanceID >= MAX_CARTA_N_INSTANCES)
		return;
	...
	for (int i = 0; i < MAX_PARTY; i++)
	{
		int partyconn = pMob[pInstance->LeaderConn].PartyList[i];

		if (partyconn > 0 && partyconn < MAX_USER && pUser[partyconn].Mode == USER_PLAY)
		{
			char msg[128];
			sprintf(msg, "Monstros restantes: %d/%d", remaining, pInstance->MobsTotal);
			SendClientMessage(partyconn, msg);
		}
	}
}

void SendWaveInfo(int instanceID)
{
	if (instanceID < 0 || instanceID >= MAX_CARTA_N_INSTANCES)
		return;
	...
	// Enviar para o líder primeiro
	if (leaderConn > 0 && leaderConn < MAX_USER && pUser[leaderConn].Mode == USER_PLAY)
	{
		SendClientSignalParm(leaderConn, ESCENE_FIELD, _MSG_StartTime, pInstance->WaveTime);
		char msg[128];
		sprintf(msg, "Onda %d iniciada! Monstros: %d - Tempo: %d segundos", pInstance->CurrentWave, pInstance->MobsTotal, pInstance->WaveTime);
		SendClientMessage(leaderConn, msg);
	}

	// Enviar para o resto do grupo
	for (int i = 0; i < MAX_PARTY; i++)
	{
		int partyconn = pMob[leaderConn].PartyList[i];
		...
		{
			SendClientSignalParm(partyconn, ESCENE_FIELD, _MSG_StartTime, pInstance->WaveTime);
			char msg[128];
			sprintf(msg, "Onda %d iniciada! Monstros: %d - Tempo: %d segundos", pInstance->CurrentWave, pInstance->MobsTotal, pInstance->WaveTime);
			SendClientMessage(partyconn, msg);
		}
