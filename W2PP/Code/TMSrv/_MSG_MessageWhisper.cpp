...
void Exec_MSG_MessageWhisper(int conn, char* pMsg)
{
	...
	std::regex int_regex1("^[A-Za-zÃÂ-ÃÂº0-9/+ -@_^~]{0,16}$");
	std::regex int_regex2("^[A-Za-zÃÂ-ÃÂº0-9/+ -@_^~]{0,100}$");
	...
	if (command == "grupotransferir")
	{
		...
		if (!Find)
		{
			SendClientMessage(conn, "Este jogador nÃÂ£o pertence ao seu grupo.");
			return;
		}
	...
	if (strcmp(m->MobName, "entrar") == 0)
	{
		...
		if (sscanf_s(m->String, "%15s %6s", TargetName, _countof(TargetName), Password, _countof(Password)))
		{
			if (sizeof(m->MobName) <= 0 || sizeof(m->MobName) > 16) {
				SendClientMessage(conn, "Nome invÃÂ¡lido.");
				return;
			}

			if (sizeof(Password) <= 0 || sizeof(Password) > 7) {
				SendClientMessage(conn, "Senha invÃÂ¡lida.");
				return;
			}
			...
			{
				SendClientMessage(conn, "Personagem nÃÂ£o encontrado");
				return;
			}
			if (pMob[idx].Leader != 0)
			{
				SendClientMessage(conn, "Este personagem nÃÂ£o ÃÂ© lider de grupo.");
				return;
			}
			if (conn == idx)
			{
				SendClientMessage(conn, "NÃÂ£o ÃÂ© possivel.");
				return;
			}

			if (pMob[conn].Leader != 0)
			{
				SendClientMessage(conn, "VocÃÂª jÃÂ¡ faz parte de um grupo.");
				return;
			}
			for (int i = 0; i < MAX_PARTY; i++)
			{
				...
				{
					SendClientMessage(conn, "VocÃÂª ÃÂ© lider de um grupo.");
					return;
				}
			}
...
#pragma region >> Aposta Pista +6
	...
	else if (strcmp(m->MobName, "@aposta1") == 0)
	{
		if (pMob[conn].TargetX > 3330 && pMob[conn].TargetY > 1475 && pMob[conn].TargetX < 3448 && pMob[conn].TargetY < 1525)
		{
			...
			{
				SendClientMessage(conn, "Somente o lÃÂ­der.");
				return;
			}
			if (pMob[conn].extra.EMPTY[9] == 1)
			{
				SendClientMessage(conn, "Aposta jÃÂ¡ foi feita.");
				return;
			}
...
#pragma endregion 
	...
	//ocultar palavrÃÂ£o dentro do servidor
//#pragma region filtro de palavras
//	char Palavroes[34][34] = { { "porra" }, { "merda" }, { "cacete" }, { "bosta" }, { "vtnc" }, { "viado" }, { "cu" }, { "fdp" }, { "cuzÃÂ£o" }, { "rapariga" }, { "server" }, { "negro" }, { "preto" }, { "buceta" }, { "legends" }, { "bucetinha" }, { "over" }, { "kod" }, { "destiny" }, { "star" }, { "serve" }, { "lixo" }, { "adm" }, { "gm" }, { "admin" }, { "bug" }, { "bugado" }, { "lixin" }, { "puta" }, { "legacy" }, { "dynasty" }, { "dinasti" }, { "dinasty" }, { "rollback" } };
//
//	for (int i = 0; i < 34; i++)
//
//		if (strstr(m->String, Palavroes[i]) != NULL)
//		{
//			SendClientMessage(conn, "Sua digitaÃÂ§ÃÂ£o tem uma palavra Irregular. [NÃÂO ENVIADO]");
//			return;
//		}
//#pragma endregion 
...
#pragma region /novato 
		...
		{
			SendClientMessage(conn, "Sua recompensa aparecerÃÂ¡ na tela quando alcanÃÂ§ar o level necessÃÂ¡rio");
			return;
			...
			//	if (invfree < 2)
			//	{
			//		SendClientMessage(conn, "Seu inventÃÂ¡rio estÃÂ¡ cheio.");
			//		return;
			//	}
			//	// shire 3 dias
			//	Item1.sIndex = 3980;
			...
			//	PutItem(conn, &Item1);
			//	SendClientMessage(conn, "ParabÃÂ©ns iniciante, sua recompensa chegou ao seu inventÃÂ¡rio.");
			//	pUser[conn].chave1 = 1;
			//	SendEtc(conn);
			//	SaveUser(conn, 0);
			//	return;
			...
			//else
			//{
			//	SendClientMessage(conn, "Comando jÃÂ¡ utilizado nesta conta!");
			//	return;
			//}
		}
#pragma endregion  
...
#pragma region InfoPlayer
		else if (!strcmp(m->MobName, "infoplayer"))
		{
			...
			if (Id != 0)
				...
				SendClientMessage(conn, "Player nÃÂ£o estÃÂ¡ online ou nÃÂ£o foi encontrado");
			return;
			}
#pragma endregion
#pragma region >> Comando GM Via IpMac
		//adm on - APENAS para contas autorizadas
		else if (!strcmp(m->MobName, "adm"))
		{
			// SECURITY: Verificar se a conta esta autorizada (case-insensitive)
			bool isAuthorizedAdmin = false;
			char accName[64];
			strncpy(accName, pUser[conn].AccountName, sizeof(accName));

			// Converter para lowercase
			for (int i = 0; accName[i]; i++) {
				if (accName[i] >= 'A' && accName[i] <= 'Z')
					accName[i] += 32;
			}

			if (strcmp(accName, "felipe") == 0 || strcmp(accName, "felipe2") == 0) {
				isAuthorizedAdmin = true;
			}

			if (!isAuthorizedAdmin) {
				SendClientMessage(conn, "Voce nao tem permissao para usar este comando.");
				SystemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, "TENTATIVA DE USAR COMANDO ADM SEM PERMISSAO");
				return;
			}
			...
			if (!strcmp(m->String, "on"))
			{
				if (pUser[conn].Admin == 0)
				{
					...
					SendScore(conn);
					SystemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, "ADMIN ATIVADO");
					return;
				}
				if (pUser[conn].Admin == 1)
				{
					...
					SendScore(conn);
					SystemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, "ADMIN DESATIVADO");
					return;
				}
			}
			else
			{
				SendClientMessage(conn, "Use: /adm on");
				return;
			}
			return;
		}
...
#pragma endregion
	if (strcmp(m->MobName, "uxmal") == 0)
	{
		...
		{
			SendClientMessage(conn, "ImpossÃÂ­vel se teleportar nesta Area");
			return;
		}
	...
	if (strcmp(m->MobName, "@levelitem") == 0)
	{
		...
		if (Recompensa == 0) {
			SendClientMessage(conn, "VocÃÂª jÃÂ¡ retirou todas as recompensas para seu level");
			return;
		}
		if (Recompensa == 1) {
			STRUCT_ITEM Item1, Item2;
		...
		if (Recompensa == 2) {
			...
			if (invfree < 3)
			{
				SendClientMessage(conn, "NÃÂ£o hÃÂ¡ espaÃÂ§o suficiente no inventÃÂ¡rio");
				return;
			}
			...
			PutItem(conn, &Item1);
			SendClientMessage(conn, "ParabÃÂ©ns por alcanÃÂ§ar o Lv114!");
			pUser[conn].chave1 = Recompensa;
			SendEtc(conn);
			SaveUser(conn, 0);
			return;
		...
		if (Recompensa == 3) {
			...
			SendClientMessage(conn, "ParabÃÂ©ns por alcanÃÂ§ar o Lv125!");
			pUser[conn].chave1 = Recompensa;
			SendEtc(conn);
			SaveUser(conn, 0);
			return;
		...
		if (Recompensa == 4) {
			...
			SendClientMessage(conn, "ParabÃÂ©ns por alcanÃÂ§ar o Lv144!");
			pUser[conn].chave1 = Recompensa;
			SendEtc(conn);
			SaveUser(conn, 0);
			return;
		...
		if (Recompensa == 5) {
			...
			SendClientMessage(conn, "ParabÃÂ©ns por alcanÃÂ§ar o Lv256!");
			pUser[conn].chave1 = Recompensa;
			SendEtc(conn);
			SaveUser(conn, 0);
			return;
...
#pragma region >> Info Mac
	...
	else if (strcmp(m->MobName, "infoplay") == 0) //pega as informaÃÂ§ÃÂ£o do jogador dentro do servidor
	{
		int MacAlvo = 0;
...
#pragma region /criar
	else if (strcmp(m->MobName, "create") == 0 || strcmp(m->MobName, "Create") == 0 || strcmp(m->MobName, "criar") == 0 || strcmp(m->MobName, "Criar") == 0)
	{
		...
		{
			SendClientMessage(conn, "VocÃÂª precisa ser Capa Azul ou Red para criar sua guild.");
			return;
		}
		...
		for (int i = 0; i < 65535; i++)
		{
			...
			{
				SendClientMessage(conn, "Este nome jÃÂ¡ estÃÂ¡ em uso!");
		...
		// SECURITY: SQL Escape para nome da guild (m->String)
		char escapedGuildName[NAME_LENGTH * 2 + 1];
		cSQL::EscapeString(escapedGuildName, m->String, sizeof(escapedGuildName));

		SNPRINTF_SAFE(xQuery, sizeof(xQuery), "INSERT INTO guilds (guild, slug, mark, kingdom, name) VALUES ('%d','%d','%d','%d','%s')",
			usGuild, usGuild, usGuild, pMob[conn].MOB.Clan, escapedGuildName);
		pc.wQuery(xQuery);
...
#pragma endregion
	...
	if (strcmp(m->MobName, "filtro") == 0) {
		...
		if (pMob[conn].MOB.Equip[13].sIndex != 3901 && pMob[conn].MOB.Equip[13].sIndex != 3902 && pMob[conn].MOB.Equip[13].sIndex != 3916) {
			SendClientMessage(conn, "Precisa de uma fada azul/vermelha ou do vale para usar essa funÃÂ§ÃÂ£o!");
			return;
		}
	...
	/*else if (strcmp(m->MobName, "pin") == 0 || strcmp(m->MobName, "PIN") == 0)
	{
		...
		else
		{
			...
			if (fp)
			{
				...
				char tmg[256];
				SendClientMessage(conn, "Agradecemos a sua contribuiÃÂ§ÃÂ£o... Digite /saldo");
				SaveUser(conn, 0);
				SendEtc(conn);
				return;
			}
			else
				SendClientMessage(conn, "Um erro ocorreu durante a ativaÃÂ§ÃÂ£o, favor contatar administraÃÂ§ÃÂ£o.");
		}
		return;
	}*/
#pragma endregion 
