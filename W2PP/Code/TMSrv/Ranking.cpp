...
void doRanking(int conn)
{
	...
	// SECURITY: SQL Escape para MobName
	char escapedName[NAME_LENGTH * 2 + 1];
	cSQL::EscapeString(escapedName, pMob[conn].MOB.MobName, sizeof(escapedName));

	SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE `characteres` SET level = '%d', class = '%d', evolution = '%d', kingdom = '%d', frags = '%d', guild_id = '%d', guildlevel = '%d' WHERE `nick` = '%s' AND `slot_char` = '%d'",
		pMob[conn].MOB.BaseScore.Level, pMob[conn].MOB.Class, Evolution, pMob[conn].MOB.Clan,
		Kills, pMob[conn].MOB.Guild, pMob[conn].MOB.GuildLevel, escapedName, pUser[conn].Slot);
	pc.wQuery(xQuery);
}
...
void sendRanking(int conn, int state) {
	...
	if (state == 0) {
		SNPRINTF_SAFE(xQuery, sizeof(xQuery), "SELECT * FROM `characteres` where level is not null ORDER BY `evolution` DESC, `level` DESC LIMIT 50");
		MYSQL_ROW row;
	...
	if (state == 1) {
		SNPRINTF_SAFE(xQuery, sizeof(xQuery), "SELECT * FROM `characteres` where level is not null ORDER BY `frags` DESC LIMIT 50");
		MYSQL_ROW row;
		MYSQL* wSQL = pc.wStart();
		MYSQL_RES* result = pc.wRes(wSQL, xQuery);
