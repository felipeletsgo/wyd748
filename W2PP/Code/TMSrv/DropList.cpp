...
void UpdateDropList(int conn, const char* mobname, int region)
{
	...
	char escapedMobname[NAME_LENGTH * 2 + 1];
	cSQL::EscapeString(escapedMobname, mobname, sizeof(escapedMobname));

	SNPRINTF_SAFE(xQuery, sizeof(xQuery), "SELECT * FROM `droplist` WHERE `nome` = '%s'", escapedMobname);
...
void ReqdListNames(int conn, char* pMsg)
{
	...
	SNPRINTF_SAFE(xQuery, sizeof(xQuery), "SELECT * FROM `droplist` WHERE `region` = %d LIMIT 30", region);
