...
MYSQL* cSQL::wStart()
{
	...
	try
	{
		...
		if (!mysql_real_connect(wSQL, HOST, USER, PASS, DB, PORT, NULL, CLIENT_MULTI_STATEMENTS))
		{
...
MYSQL_RES *cSQL::wRes(MYSQL *sql, char* query)
{
	try {
		...
		{
			printf("[wMySQL][wRes] Erro na execu��o da wRes.\nQuery: %s\n\t\tErro: %s\n", query, mysql_error(sql));
			mysql_close(sql);
			return NULL;
		}
...
void cSQL::wLog(char* acc, char* pers, char* mensagem, char* type)
{
	...
	{
		printf("[wMySQL][Log] Erro na execu��o da wQuery.\n\t\tErro: %s\n", mysql_error(wSQL));
		mysql_close(wSQL);
		//ExitThread(0);
		return;
	}
...
bool cSQL::wQuery(char* query)
{
	try
	{
		...
		{
			printf("[wMySQL][wQuery] Erro na execu��o da wQuery.\n\t\tErro: %s\n", mysql_error(wSQL));
			mysql_close(wSQL);
			//ExitThread(0);
			return FALSE;
		}
...
void cSQL::EscapeString(char* dest, const char* src, int dest_size)
{
	...
	if (mysql)
	{
		mysql_real_escape_string(mysql, dest, src, strlen(src));
		mysql_close(mysql);
	}
	else
	{
		// Fallback: copia seguro se mysql_init falhar
