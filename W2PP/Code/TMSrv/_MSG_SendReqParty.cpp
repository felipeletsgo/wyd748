...
void Exec_MSG_SendReqParty(int conn, char* pMsg)
{
	...
	//if (pUser[conn].AtrasoParty != 0)
	//{
	//	timer isTime = tick - pUser[conn].AtrasoParty; // ATRASO PACOTE DE MORTE

	//	if (isTime < 2000)
	//	{
	//		pUser[conn].AtrasoParty = tick;
	//		return;
	//	}
	//}
	//pUser[conn].AtrasoParty = tick;
	...
	{
		{
			if (index >= MAX_PARTY)
			{
				...
				if (conn >= MAX_USER || pMob[conn].TargetX < 2604 || pMob[conn].TargetY < 1708 || pMob[conn].TargetX > 2648 || pMob[conn].TargetY > 1744)
				{
