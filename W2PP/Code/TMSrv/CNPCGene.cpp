...
int CNPCGenerator::ReadNPCGenerator()
{
	...
	if (fs == NULL)
	{
		MessageBoxA(hWndMain, "NPCGener.txt�� �", "+-��+� ����", MB_OK);
	...
	for (int i = 0; i < NumList; i++)
	{
		...
		{	
			...
			if (region != 0) {
				...
				cSQL::EscapeString(escapedName, pList[i].Leader.MobName, sizeof(escapedName));

				SNPRINTF_SAFE(xQuery, sizeof(xQuery), "INSERT INTO `droplist` (`nome`) VALUES ('%s')", escapedName);
				pc.wQuery(xQuery);
			}
			...
			SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE droplist SET region = '%d' WHERE nome = '%s'", region, escapedName);
			pc.wQuery(xQuery);
			...
			{
				SNPRINTF_SAFE(xQuery, sizeof(xQuery), "UPDATE droplist SET slot%d = '%d' WHERE nome = '%s'", k, MOB->Carry[k].sIndex, escapedName);
				pc.wQuery(xQuery);
