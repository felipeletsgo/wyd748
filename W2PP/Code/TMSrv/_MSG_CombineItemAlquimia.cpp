...
void Exec_MSG_CombineItemAlquimia(int conn, char *pMsg)
{
	...
	{
		SendClientMessage(conn, "Imposs�vel executar a��o14, tente mais tarde. ");
		return;
	}
	...
	{
		SendClientMessage(conn, "A Composi��o desses itens foi Desativada");
		return;
	}*/

	if (combine == 0)
	{
		SendClientMessage(conn, "Composi��o n�o permitida com itens em pack");
		return;
	}
	...
	for (int i = 0; i < MAX_COMBINE; i++)
	{
		if (m->Item[i].sIndex)
		{
			...
			STRNCAT_SAFE(temp, itemlog, sizeof(temp));
		}
	}
