...
void Exec_MSG_DeleteCharacter(int conn, char *pMsg)
{
	...
	// SECURITY: Validar caracteres perigosos no nome
	const char* dangerous_chars = "'\"\\;--";
	for (const char* c = dangerous_chars; *c; c++)
	{
		if (strchr(m->MobName, *c) != NULL)
		{
			SendClientMessage(conn, "Nome de personagem inválido.");
			SystemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, strFmt("err,deletechar invalid_chars %d %s", conn, m->MobName));
			CloseUser(conn);
			return;
		}
	}
	...
	if (pUser[conn].Mode == USER_CHARWAIT)
	{
		...
		SNPRINTF_SAFE(xQuery, sizeof(xQuery), "DELETE FROM `characteres` WHERE `nick`= '%s' AND `slot_char`= '%d'", escapedName, m->Slot);
		pc.wQuery(xQuery);
