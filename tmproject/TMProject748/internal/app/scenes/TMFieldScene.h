...
// Native WYD 7.48 FUN_00435b13 binds twelve FieldScene2 tooltip rows (0..11).
...
public:
	...
	// The 7.48 FieldScene resource has a smaller control set than the source
	// tree's newer field scene.  This path keeps world/character creation
	// independent from optional HUD controls until those controls are mapped.
	int OnPacketCreateMobCompat(MSG_STANDARD* pStd);
	int OnPacketCNFCharacterLogout(MSG_STANDARD* pStd);
	int OnPacketCNFRemoveServer(MSG_CNFRemoveServer* pStd);
	// WYD-Go sends the native account-login aggregate, whose SecretCode precedes
	// STRUCT_SELCHAR. The remove-server aggregate has a different field order.
	int OnPacketCNFAccountLogin(MSG_CNFAccountLogin* pStd);
...
private:
	...
	void UpdateCompatScoreUI();
	// Projects LearnedSkill[0] into the 24 native 7.48 skill-panel grids.
	// These controls are resource-owned; only their SGridControlItem children
	// are created and removed here.
	void UpdateCompatLearnedSkillUI();
	// Runs the native 7.48 portal-cell state machine for both scene paths.
	// The compatibility frame must execute this before its compact early return.
	int UpdateTeleportPrompt();
	// Materializes the 63 client-visible carry slots over the decorative 7.48
	// inventory panel, whose original resource contains no SGridControl object.
	void InitializeCompatInventory();
	// Recreates the two ten-slot shortcut grids used by the native 7.48 packet
	...
	// Selects one of the five native Helper tabs and clears every other page.
	// The 7.48 resource keeps the Help pages and list boxes as independent
	// siblings, so hiding only the selected parent is not sufficient.
	void SelectHelpTab(int tabIndex);
...
public:
	...
	void GetCargoCellForSlot(int slot, int& cellX, int& cellY) const;
	// Performs the inverse mapping for every C->S inventory interaction.  This
	// keeps mouse, macro, shop, trade and item-use packets on the same 7.48 slot
...
public:
	...
	SText* m_pLottoCost;
	int m_bRankTimeOn;
	// Instance HUD state is driven by the server packets (0x3A1/0x3B0), not
	// by the terrain's legacy dungeon-coordinate table.  Keep it separate from
	// the shared text controls so every instance can use the same HUD.
	int m_bInstanceRemainOn;
