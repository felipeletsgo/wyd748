...
func TestSourceSelectionUpdateAndEnterWorldMatchTMProjectABI(t *testing.T) {
	...
	ch := model.Char{Name: "ProtocolHero", X: 2100, Y: 2101, Gold: 777, Exp: 998877, Score: ext}
	...
	if len(enter) != 2104 || ParseHeader(enter).Type != OpEnterWorld || binary.LittleEndian.Uint16(enter[1242:1244]) != 9 {
		t.Fatalf("source enter-world layout invalid: len=%d header=%+v id=%d", len(enter), ParseHeader(enter), binary.LittleEndian.Uint16(enter[1242:1244]))
	}
	if got := binary.LittleEndian.Uint16(enter[1240:1242]); got != 3 {
	...
	// occupies 32 bytes before Affect[0], so the first affect is exactly @1296.
	affect := 1296
...
func TestSourceWorldPacketsKeepSourceOnlySizes(t *testing.T) {
	ext := &model.Score{Version: model.ScoreVersion, Attack: 999, MaxHP: 7000, CurHP: 6500}
	cp := int16(5)
...
func TestSourceScoreRefreshKeepsPendingCostsSeparateFromCurrentResources(t *testing.T) {
	...
	ch := model.Char{Score: ext, LearnedSkill: 0xFFFFFFFF}
	...
	if got := binary.LittleEndian.Uint16(b[236:238]); got != 65 {
	...
	if b[240] != 0 {
		t.Fatalf("source score avatar selector = %d; want zero", b[240])
	}
}
...
func TestSourceMobScoreUsesTheSourceScoreABI(t *testing.T) {
	...
	if got := [4]byte{b[222], b[223], b[224], b[225]}; got != [4]byte{10, 20, 30, 40} {
		t.Fatalf("source mob resists = %v", got)
	}
	if got := binary.LittleEndian.Uint16(b[236:238]); got != 44 {
...
func TestSourceCharListMatchesTMProjectABI(t *testing.T) {
	ext := &model.Score{Version: model.ScoreVersion, Level: 77, Attack: 1234, MaxHP: 4567, CurHP: 4000}
	char := model.Char{Name: "SourceHero", X: 2100, Y: 2101, Gold: 99, Exp: 123456, Score: ext}
	...
	if len(b) != 2360 || ParseHeader(b).Type != SourceOpCharList {
	...
	if got := binary.LittleEndian.Uint64(b[32+1240 : 32+1248]); got != 123456 {
		t.Fatalf("EXP = %d", got)
	}
	if got := binary.LittleEndian.Uint16(b[1304+3*8 : 1304+3*8+2]); got != 4011 {
		t.Fatalf("cargo item = %d", got)
	}
	if got := binary.LittleEndian.Uint32(b[2328:2332]); got != 54321 {
