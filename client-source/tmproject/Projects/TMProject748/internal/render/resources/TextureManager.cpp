...
{
	// Translate the compact 7.48 rows before the renderer indexes this table.
	if (!WYD748_LoadTextureList(
		TextureListNList_Path,
		"UI\\UITextureList.bin",
		m_stUITextureList,
		_countof(m_stUITextureList)))
	{
...
{
	// Effect tables share the same legacy 264-byte record contract as UI.
	if (!WYD748_LoadTextureList(
		EffectTextureList_Path,
		nullptr,
		m_stEffectTextureList,
		_countof(m_stEffectTextureList)))
	{
...
{
	// The 7.48 model table has 2048 compact rows; unused modern slots stay empty.
	if (!WYD748_LoadTextureList(
		MeshTextureList_Path,
		nullptr,
		m_stModelTextureList,
		_countof(m_stModelTextureList)))
	{
...
{
	// Environment textures also require field translation, not a raw fread.
	if (!WYD748_LoadTextureList(
		EnvTextureList_Path,
		nullptr,
		m_stEnvTextureList,
		_countof(m_stEnvTextureList)))
	{
...
int TextureManager::InitRenderTargetTexture()
{
	...
	// DirectX 9 forbids render-target textures in D3DPOOL_MANAGED. The imported
	// TMProject used that invalid combination, so every device rejected the
	// texture and the renderer later dereferenced an incomplete target chain.
	if (FAILED(D3DXCreateTexture(
	...
	// The upstream routine allocated the origin texture a second time into the
	// same pointer. Keeping the first allocation avoids leaking both its COM
	// texture and surface while preserving the dimensions used below.
	if (FAILED(D3DXCreateTexture(
		pRenderDevice->m_pd3dDevice,
		pRenderDevice->m_d3dsdBackBuffer.Width,
		pRenderDevice->m_d3dsdBackBuffer.Height,
