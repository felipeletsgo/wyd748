...
namespace
{
// WYD748 compatibility: Ghidra FUN_0052a737 proves Carry is one 9x7 grid; TMProject 7.59 uses four 5x3 pages.
bool UsesNative748CarryLayout()
{
    auto* fieldScene = dynamic_cast<TMFieldScene*>(g_pCurrentScene);
    return fieldScene != nullptr && fieldScene->UsesNative748InventoryLayout();
}

int ActiveCarryPageCount()
{
    return UsesNative748CarryLayout() ? 1 : 4;
}

int ActiveCarryColumnCount()
{
    return UsesNative748CarryLayout() ? 9 : 5;
}

int ActiveCarryRowCount()
{
    return UsesNative748CarryLayout() ? 7 : 3;
}

// WYD748 compatibility: packet slots remain linear 0..62 while only the control topology changes between client generations.
int ActiveCarrySlot(int page, int cellX, int cellY)
{
    return UsesNative748CarryLayout()
        ? cellX + 9 * cellY
        : 15 * page + cellX + 5 * cellY;
}

SGridControl* ActiveCarryGrid(SGridControl** grids, int page)
{
    if (grids == nullptr)
        return nullptr;
    return UsesNative748CarryLayout() ? grids[0] : grids[page];
}

// WYD748 compatibility: all hard-coded recipe checks resolve a linear slot through the same native 9-column mapping.
SGridControlItem* ActiveCarryItem(SGridControl** grids, int slot)
{
    if (slot < 0)
        return nullptr;

    const bool native748 = UsesNative748CarryLayout();
...
void CItemMix::InvClear(SGridControl** GridInvList)
{
    ...
    for (int i = 0; i < ActiveCarryPageCount(); ++i)
    {
        auto pGridInv = ActiveCarryGrid(m_pGridInvList, i);
        if (!pGridInv)
            break;

        for (int nY = 0; nY < ActiveCarryRowCount(); ++nY)
        {
            for (int nX = 0; nX < ActiveCarryColumnCount(); ++nX)
            {
...
void CItemMix::CheckInv(SGridControl** GridInvList)
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        {
            auto* carryGrid = ActiveCarryGrid(m_pGridInvList, j);
            if (carryGrid == nullptr)
                continue;
            for (int nY = 0; nY < ActiveCarryRowCount(); ++nY)
            {
                for (int nX = 0; nX < ActiveCarryColumnCount(); ++nX)
                {
                    SGridControlItem* pItem = carryGrid->GetItem(nX, nY);
                                ...
                                && m_dNeedRefer[i].dHavevolume < 1)
                            {
                                ...
                                for (int k = i; k < 8; ++k)
                                {
                                    if (!m_stCombineItem.Item[k].sIndex)
                                    {
                                        ...
                                        m_stCombineItem.CarryPos[k] = ActiveCarrySlot(j, pItem->m_nCellIndexX, pItem->m_nCellIndexY);
                                        break;
                                    }
                                }
                            }
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            if (m_dNeedRefer[i].bItemListrefer)
            {
                    ...
                    !m_pGridNeedItem[i]->m_pItemList[0])
                {
                    ...
                    for (int j = 0; j < ActiveCarryPageCount(); ++j)
                    {
                        auto grid = ActiveCarryGrid(m_pGridInvList, j);
                        if (grid == nullptr)
                            continue;
                        for (int nY = 0; nY < ActiveCarryRowCount(); ++nY)
                        {
                        ...
                        for (int ki = i; ki < 8; ++ki)
                        {
                            if (!m_stCombineItem.Item[ki].sIndex)
                            {
                                ...
                                m_stCombineItem.CarryPos[ki] = ActiveCarrySlot(Sourpage, pItem->m_nCellIndexX, pItem->m_nCellIndexY);
                                break;
                            }
                        }
...
int CItemMix::HardCode(SMessagePanel* MessagePanel, SGridControl** GridInvList)
{
    ...
    {
        auto Item = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[0]);
        
        auto Item2 = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[1]);
        if (Item
    ...
    else if (m_NPCHead == 55)
    {
        for (int i = 0; i < 2; ++i)
        {
            auto Item = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[i]); // v22
        ...
        auto Item1 = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[0]); // v22

        auto Item2 = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[1]); // v22
    ...
    {
        auto Item = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[0]); // v7
        ...
        auto Item2 = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[1]); // v8
    ...
    {
        auto Item = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[0]); // v27

        auto Item2 = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[1]); // v25
            ...
            || Item->m_pItem->sIndex == 1714)
        {
            ...
            auto Item3 = ActiveCarryItem(GridInvList, m_stCombineItem.CarryPos[2]);
