...
// WYD-Go 7.48: centralize legacy control IDs so every UI action reaches the modern dispatcher consistently.
...
// The compatibility handlers below are intentionally kept together because
// they share scene ownership, packet dispatch, and teardown state.  Pure UI
// positioning helpers are isolated by the PositionCompat* naming convention;
// moving those helpers to another translation unit would require exposing
...
static int GetWYD748AttackVisualDamage(const MSG_Attack* pAttack, int index)
{
	...
	const unsigned int packetSize = pAttack->Header.Size;
	unsigned int realDamage = 0;

	// A compact 0x39D/52 packet is the physical-hit extension and carries its
	// single real damage immediately after the native 48-byte prefix.
	if (pAttack->Header.Type == MSG_Attack_One_Opcode && packetSize == 52)
	{
		memcpy(&realDamage, bytes + 48, sizeof(realDamage));
	}
	else
	{
		unsigned int baseSize = 0;
		if (pAttack->Header.Type == MSG_Attack_One_Opcode)
			baseSize = 48;
		else if (pAttack->Header.Type == MSG_Attack_Two_Opcode)
			baseSize = 52;
		else if (pAttack->Header.Type == MSG_Attack_Multi_Opcode)
			baseSize = 96;

		// Skill results use a self-describing DMGX tail: signature, count and a
		// uint32 vector. Validate every boundary before reading untrusted bytes.
		if (!baseSize || packetSize < baseSize + 8)
			return legacyDamage;

		unsigned int signature = 0;
		unsigned int count = 0;
		memcpy(&signature, bytes + baseSize, sizeof(signature));
		memcpy(&count, bytes + baseSize + 4, sizeof(count));
		if (signature != 0x58474D44 || count > 13 || static_cast<unsigned int>(index) >= count ||
			packetSize < baseSize + 8 + count * sizeof(unsigned int))
			return legacyDamage;

		memcpy(&realDamage, bytes + baseSize + 8 + index * sizeof(unsigned int), sizeof(realDamage));
	}

	// TMProject is a 32-bit client. Reject values that cannot be represented by
	// its existing signed rendering path instead of wrapping into healing/MISS.
