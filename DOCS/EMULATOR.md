...
[ticker 200ms] -------------------------------------------------/     (único dono do estado)
```
...
| 0x20F | CreateCharacter | 48 | slot@12, nome@16 e classe@32; validacao autoritativa no servidor |
| 0x213 | CharacterLogin | 36 | entrar no mundo (slot selecionado) |
| 0x366 | Action (walk) | 52 | **PosX@12, PosY@14** (posição atual; manda contínuo ao andar) |
...
Name@16 (16B — **MobName[12]=@28 = byte de chaos, pôr 150 = nome branco**), Class@36,
...
**MobName[12]=@30 = chaos, 150 se self spawn=2**), Chaos@30/CurKill@31/TotKill@32,
...
**0x36B SelfEquip (44B):** 16 WORDs @12 (sIndex de cada slot de equip = visual).
...
3. **Cor do nome = MobName[12] (byte de chaos/PK).** O client colore o nome por uma
   paleta indexada por esse byte: `<10` = normal (preto c/ borda), `~10-30` = **VERMELHO**
   (PKer), `~150` = **branco**. O server 759 manda `MobName[12] = PK points` (0=normal),
   mas o client 7.48 mostra vermelho com 0 → mandamos **150** (enter-world @28 +
...
9. **Movimento 7.48 = 0x366 contínuo**. `0x36C` fica reservado às skills
   multi-alvo de 96 bytes. O client anda mandando 0x366 com a
   posição atual @12; o servidor rastreia dela.
...
NOMEADOS (mesmos nomes da aparência dos NPCs: `rosto, elmo, armadura, calca, luva, bota,
arma, escudo, manto, acessorio1-6, montaria`); o JSON preserva as 64 entradas
...
carrega a definição inteira e cruza nomes de `Itemname.csv`; `SkillData.csv`
...
posição, Unique, preço, efeitos estáticos e nomes. `SkillData.csv` fornece custo,
alvo, mana, delay, range, instance, tick, affect e flags; `AffectTime` do arquivo
é convertido por `/4`. `normalizeTKBuff759` substitui a semântica de Samaritano,
Possuído, Aura da Vida, Assalto e Armadura Crítica pelos dados 7.59 sem mudar os
índices visuais do 7.48.

`volatiles.json` aceita `none`, `restore`, `ground_gold`, `gold`, `teleport` e
`generic`. Os efeitos `EF_VOLATILE` são descobertos no catálogo; regras por item
...
item persistido, aplica refino e só depois passivas/affects. `dynamicEffectName`
mapeia os efeitos relevantes. Sanc 230..253 representa refino +10..+15. Até +10
...
e envia 0x3B9 somente ao dono. Buffs automáticos só são renovados abaixo de 10 s
...
`{TargetID,Damage}` a partir de @44. O dano enviado é o mesmo dano autoritativo
aplicado no estado; a cor física/mágica é decidida pela codificação da skill.
...
`World`. A persistência usa `JSONStore.SaveAccounts`: um journal com marcador
`COMMIT` torna a gravação das duas contas recuperável após queda entre os dois
renames. Os clients recebem `0x185 UpdateCarry` somente depois do commit.
...
- Códigos ainda não portados usam `generic`: cada clique é catalogado e loga
  código, item, origem, destino e GridX/Y, mas não consome nem altera o item.
...
válido se o processo cair durante a gravação.
...
`Store` expõe Load/Save/CharacterNameExists; `JSONStore` acrescenta criação e
busca case-insensitive. Save usa temporário + Sync + rename, preservando o último
arquivo válido. Trades usam uma transação recuperável para duas contas.

### Constantes tunáveis principais

| Constante | Valor | Responsabilidade |
|---|---:|---|
| world tick | 200 ms | IA, movimento, affects e manutenção |
...
| renovação automática | abaixo de 10 s | buff equipado na barra |
...
internal/
  ...
  store/store.go       interface Store + jsonStore (data/accounts/<n>.json)
  data/
    data.go            LoadNPCs (pasta data/npcs/*.json; carry limitado a 64 slots)
    catalog.go         LoadCatalog (itemlist + Itemname + SkillData)
...
passivas e summons das 96 skills;
efeitos públicos e ícones/timers privados; persistência JSON atômica.
