...
O `data/itemlist.csv` traz **116 códigos `EF_VOLATILE` distintos em 347 itens**. Cada
...
override `items[itemID]`). Estado atual: **45 códigos com comportamento real, 19
ações**; os demais caem no `generic` (logam, não consomem) — parte pendente, parte
...
| Baú/poeira de EXP | 7, 8 | `grant_exp` |
...
| Volatile desativado (consome sem efeito) | 9 | `disabled` |

Pendente **dentro do escopo do plano** (Fase D + stragglers):

- **Summon** (41, 42, 43, 46, 47, 48): `Hired_Soldier_Contract`, `Guard_Summon_Letter`
  — evocar soldado/guarda, reusar `castSummon`.
- **Livros de skill Sephira** (32-36): `Conjure_Cannon`, `Thorn_Wall`, `Resurrect`,
  `Cancellation`, `Spectral_Force` — conceder a skill via `SecondaryLearnedSkill`.
- **Buff 58** (`Bigger_Potion`, `Health_Potion(7/15days)`): a família 52-67 pulou o 58.
- **Verificar**: 6 (`Magical_Pill`), 195 (`Hunting_Scroll`, teleporte-pra-zona).

Fora de escopo desta rodada (permanecem no `generic` **de propósito**): dungeons e
instâncias (3 Gate_Keys, 51 Entrance_Ticket, 54 Cube, 131-178, 211, 212), fireworks
(19), economia/misc (18, 184, 187-191, 203, 206-208) e os "não sei" (179, 192, 193,
197). Um item novo com `EF_VOLATILE` sem regra cai no `generic` e é logado — nada
passa despercebido.
