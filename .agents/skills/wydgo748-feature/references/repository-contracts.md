# Contratos detalhados do repositório WYD-Go

> Referência detalhada carregada sob demanda. O `AGENTS.md` da raiz é o
> roteador canônico e prevalece em caso de conflito.
...
```text
tmproject/client748/skills/wyd-client-assets/SKILL.md
```
...
implementação, testes, GitHub, PRs, CI e adaptação da source do client.
...
```text
.agents/skills/wydgo748-feature/SKILL.md
```
...
- análise histórica de hooks/trampolines e ABI do `WYD.exe`;
- itens e volatiles;
- skills;
- mobs;
- bosses;
...
`SKILL.md` atual da `wydgo748-feature`.**
...
Para importar, corrigir ou auditar qualquer asset visual sob `tmproject/client748/`, ler
também:

```text
tmproject/client748/AGENTS.md
tmproject/client748/skills/wyd-client-assets/SKILL.md
```

Esse workflow cobre MSH/WYS/BON/ANI, `MeshTextureList.bin`, `ValidIndex.bin`,
selector/renderer de costumes e montarias na source recompilável 7.48.
Separar pesquisa read-only, exploração, implementação e integração conforme os
papéis definidos no `tmproject/client748/AGENTS.md`.
...
Para qualquer tarefa relacionada ao WYD, a leitura de `.agents/skills/wydgo748-feature/SKILL.md` é pré-condição obrigatória, conforme a seção **Uso obrigatório de skills**.
...
fronteira legada em `tmproject/`, protocolo, ABI, structs, UI, input, render
ou assets consultam a descompilação Ghidra do binário histórico
`references/client748/WYD.exe`. O agente deve ler também
`.agents/skills/wydgo748-feature/references/ghidra-client748.md` e registrar as
...
`tmproject` não é uma source multi-versão. O produto dessa árvore
...
Não tratar os executáveis históricos e o candidato recompilado como artefatos
intercambiáveis:

- `references/client748/WYDoriginal.exe` é o stock histórico
  imutável;
- `references/client748/WYD.exe` é a referência histórica da análise
  Ghidra;
- `tmproject/build/<Configuration>/WYD.exe` é apenas a saída
  transitória da compilação da source;
- `tmproject/client748/project.exe` é o único candidato executável e de validação,
  instalado diretamente a partir da source recompilada.

Os scripts e patches dentro de `references/client748/` são somente
material de estudo: nunca executá-los ou editá-los. Toda alteração ativa deve
ser implementada em `tmproject/` ou nos assets e instalada diretamente como
`tmproject/client748/project.exe`, sem patch binário.

## Controles de UI inexistentes no 7.48

A existência de um controle é determinada pelo fluxo nativo 7.48 e pelo recurso
de UI efetivamente carregado, não pela presença de um campo herdado da source
7.59. Quando um controle moderno não existir no recurso 7.48:

- remover a dependência funcional desse controle ou manter o ponteiro opcional;
- proteger todos os acessos, inclusive updates indiretos de score/equip/montaria;
- preservar a transição principal de estado mesmo sem o widget opcional;
- não fabricar um controle apenas para ocultar `nullptr` ou silenciar crash.

Um binding ausente pode ser o contrato correto do 7.48. Validar IDs exatos e,
quando o formato os truncar, também os IDs convertidos para a largura realmente
consumida pelo loader.
...
# Source e assets do client 7.48

Patches no executável não fazem parte do produto nem da validação. Os binários,
scripts, hooks e trampolines históricos podem ser inspecionados somente para
recuperar semântica 7.48; qualquer comportamento necessário deve ser refeito em
`tmproject/` ou representado nos assets consumidos pelo `project.exe`.
...
Ao estudar cada hook/trampoline histórico antes de adaptar sua semântica:
...
Um teste estático de bytes históricos não substitui build nem teste runtime do
`project.exe`.

Toda adaptação ativa deve:

- implementar o contrato 7.48 em código C++ compilável ou em asset suportado;
- evitar endereços absolutos, code caves e dependência de bytes do executável;
- compilar por `tmproject/Build-Client.ps1`;
- exigir que o próprio build instale e confira automaticamente
  `tmproject/client748/project.exe`;
- registrar o hash do candidato e testar o fluxo real afetado;
- nunca executar scripts sob `references/client748/`.
...
Manuais ativos que permanecem junto do artefato descrito:

```text
data/boss/README.md
```

`references/client748/PATCHES.md` documenta apenas a cadeia
histórica read-only. Não é manual de build, implementação, validação ou
recuperação do client ativo.
