...
- `client748/wyd.exe nativo+patches/WYDoriginal.exe` é o stock histórico
  imutável;
- `client748/wyd.exe nativo+patches/WYD.exe` é a referência histórica da análise
  Ghidra;
- `client-source/tmproject/build/<Configuration>/WYD.exe` é apenas a saída
...
Os scripts e patches dentro de `client748/wyd.exe nativo+patches/` são somente
material de estudo: nunca executá-los ou editá-los. Toda alteração ativa deve
ser implementada em `client-source/` ou nos assets e instalada diretamente como
...
`client-source/` ou representado nos assets consumidos pelo `project.exe`.
...
- compilar por `client-source/tmproject/Build-Client.ps1`;
...
- nunca executar scripts sob `client748/wyd.exe nativo+patches/`.
...
`client748/wyd.exe nativo+patches/PATCHES.md` documenta apenas a cadeia
